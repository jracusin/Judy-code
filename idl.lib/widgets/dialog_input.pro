...
FUNCTION DIALOG_INPUT, PROMPT = prompt, TITLE = title, INITIAL = initial, $
    ...
    FLOATING = FLOATING, INTEGER = INTEGER,  LONG = LONG, $
    NFIELDS = nfields, DIALOG_PARENT = dialog_parent, $
    OK = ok, CANCEL = cancel, RETURN_EVENTS=return_events
...
IF (HAVE_PARENT) THEN BEGIN
   topBase = WIDGET_BASE (TITLE = title, /COLUMN, /BASE_ALIGN_CENTER, $
       ...
       XSIZE = xsize, YSIZE = ysize, resource_name='colorbuttons')
ENDIF ELSE BEGIN
...
IF (numFields EQ 1) THEN BEGIN
   ...
   FOR i = 0,  numFields - 1 DO $
       textID[i] = CW_FIELD (base, VALUE = value[i], XSIZE = width, $
           FLOATING = FLOATING, INTEGER = INTEGER, RETURN_EVENTS=Return_Events,$
	   LONG = LONG)
...
ENDIF ELSE BEGIN
   ...
   FOR i = 0,  N_ELEMENTS (prompt) - 1 DO BEGIN
       textID[i] = CW_FIELD (base, VALUE = value[i], XSIZE = width, $
        		FLOATING = FLOATING, INTEGER = INTEGER,  LONG = LONG, $
        		TITLE = prompt[i], RETURN_EVENTS=Return_Events)
   ENDFOR
...
IF KEYWORD_SET( Integer ) THEN retval = FIX(retval)
IF KEYWORD_SET( Long ) THEN retval = LONG(retval)
IF KEYWORD_SET( Floating ) THEN retval = FLOAT(retval)
