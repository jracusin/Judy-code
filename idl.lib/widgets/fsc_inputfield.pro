...
CASE thisEvent OF
   'Field 1 Event': BEGIN
      Print, ''
      IF N_Elements(*event.value) EQ 0 THEN Print, 'xmin is Undefined' ;ELSE $
;         Print, 'Field 1 Value: ', *event.value
      END
   'Field 2 Event': BEGIN
      Print, ''
      IF N_Elements(*event.value) EQ 0 THEN Print, 'xmax is Undefined' ;ELSE $
;         Print, 'Field 2 Value: ', *event.value
      END
;   'Field 3 Event': BEGIN
;      Print, ''
...
;      END
   'OK': BEGIN
	f1=info.field1->Get_Value()
	f2=info.field2->Get_Value()
	info.field1->Set_Value,f1
	info.field2->Set_Value,f2
;      IF N_Elements(info.field2->Get_Value()) EQ 0 THEN Print, 'Field 2 Value is Undefined' ELSE $
;         Print, 'Field 2 Value: ', info.field2->Get_Value()
...
tlb = Widget_Base(Column=1)
;button = Widget_Button(tlb, Value='xmin', $
;   UValue='xmin')
;button = Widget_Button(tlb, Value='xmax', $
;   UValue='xmax')
field1 = FSC_INPUTFIELD(tlb, Title='X min:', LabelSize=50, Digits=4, $
   Value=5, /IntegerValue, UValue='Field 1 Event', Event_Pro='Example_Event', /Focus_Events)
field2 = FSC_INPUTFIELD(tlb, Title='X max:', LabelSize=50, Digits=4, $
   Value=5, /IntegerValue, UValue='Field 2 Event', Event_Pro='Example_Event', /Focus_Events)


;field2 = FSC_INPUTFIELD(tlb, Title='X max', LabelSize=50, Digits=4, /CR_Only, $
;   /FloatValue, UValue='Field 2 Event', Event_Pro='Example_Event', /Positive)
;field3 = FSC_INPUTFIELD(tlb, Title='String:', LabelSize=50, Value='Coyote Rules!', $
;   UValue='Field 3 Event', /Focus_Events, Event_Pro='Example_Event')
...
button = Widget_Button(tlb, Value='OK', UValue="OK")
button = Widget_Button(tlb, Value='Cancel', UValue='Cancel')
;button = Widget_Button(tlb, Value='Change Size of Labels', UValue='LabelSize')
;button = Widget_Button(tlb, Value='Print Floating Value', UValue='PrintFloat')
;button = Widget_Button(tlb, Value='Quit', UValue='Quit')

Widget_Control, tlb, /Realize, Set_UValue={field1:field1, field2:field2}
XManager, 'example2', tlb, /No_Block
stop
END
