...
;
; CALLING SEQUENCE:
;      Result = RDPFITS( Filename/FileUnit,[Extension, Header],
...
;                      repeat begin
;                          thisHDU = RDPFITS(lun, 0, hdr, status=status)
;                          ... process the HDU ...
...
;                This was only behavior prior to V2.5 or RDPFITS and remains
...
;                is specified RDPFITS will ignore TDIM keywords in
;                binary tables.
...
;                in calls to RDPFITS in the same IDL session for extensions
...
;                RDPFITS attempts to use the appropriate TTYPE keyword
...
;       RDPFITS DOES NOT scale data by default.  The FSCALE or DSCALE
;       parameters must be used.
;
;       As of version 2.5 RDPFITS support 64 bit integer data types.
;       These are not standard FITS.  64 bit data also requires
;       IDL version 5.2 or greater.
;    
;
; PROCEDURES USED:
;       The following procedures are contained in the main RDPFITS program.
...
;          - Modified RDPFITS to read in headers containing null characters
...
;          In this case RDPFITS returns a scalar 0.
;          - Fix bugs with the values for BSCALE and BZERO (and PSCAL and
;          PZERO) parameters set by RDPFITS.
...
;       Aug. 16, 2004: Copied RDPFITS from MRDFITS. The only change is to 
;                          call RDP_STRUCT, instead of MRD_STRUCT, which uses
;                          EXECUTE. 
;       V2.7 Mar. 30, 2005: Fixed handling of variable-length string arrays. Now 
;                          they are read into bytearrays, so the user must convert.
;                          I see no easy way out, so I list this as a TODO. 
;
...
; Return the currrent version string for RDPFITS
...
pro mrd_ascii, header, structyp, use_colnum,   $
    ...
    if strmid(fxpar(header, 'XTENSION'),0,8) ne 'TABLE   ' then begin
        print, 'RDPFITS: Header is not from ASCII table.'
        status = -1;
        return
    endif
    ...
    if nrows le 0 then begin
        if not keyword_set(silent) then begin
            print,'RDPFITS: ASCII table.  ',strcompress(string(nfld)),  $
                  ' columns, no rows'
        endif
        return
    endif
	...
	if cnt eq 0 then fnull = ''
        ...
        print, 'RDPFITS: Invalid format code:',ftype, ' for column ', i+1
        status = -1
        return
  next_col:
    endfor
    ...
    if not scaling and not keyword_set(columns) then begin
        table = rdp_struct(fnames, fvalues, nrows, structyp=structyp, $
          tempdir=tempdir, silent=silent, old_struct=old_struct)
    endif else begin
        table = rdp_struct(fnames, fvalues, nrows, tempdir=tempdir, $
...
pro  mrd_columns, table, columns, fnames, fvalues, $
    ...
    if n_elements(tcols) le 0 then begin
        print, 'RDPFITS:  No columns match'
        ...
        if scaling or n_elements(vcls) gt 0 then begin
            tabx = rdp_struct(fnames, fvalues, ndim, tempdir=tempdir, $
              silent=silent, old_struct=old_struct)
        endif else begin
            tabx = rdp_struct(fnames, fvalues, ndim, structyp=structyp, $
              tempdir=tempdir, silent=silent, old_struct=old_struct)
        endelse
    ...
    for i=naxis-1,1,-1 do begin 
        ...
        if dims[i] eq 1 then begin
            if not mysilent then begin
                print, 'RDPFITS: Truncating unused dimensions'
                mysilent = 1
            endif
            dims = dims[0:i-1] 
            naxis = naxis - 1 
...
pro mrd_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
  status, silent=silent, tempdir=tempdir, unsigned=unsigned
	...
	print,'RDPFITS: Error: Invalid BITPIX: '+strtrim(bitpix)
	table = 0
	return
    endif
	...
	maxd = gcount
        ...
        if (n_elements(dims) gt 1) then begin
            ...
            print, 'RDPFITS: Warning: No data specified for group data.'
            dims = [0]
            naxis = 0
        endelse
        ...
        if not keyword_set(silent) then print,'RDPFITS--Image with groups:', $
        ...
        table = rdp_struct(['params','array'], values, range[1]-range[0]+1, $
          tempdir=tempdir, silent=silent)

	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Reset by RDPFITS v'+mrd_version()
	endif
        ...
        if naxis eq 0 then begin
            ...
            table = 0
            if not keyword_set(silent) then begin
                print, 'RDPFITS: Null image, NAXIS=0'
            endif
            return
        ...
        if not keyword_set(silent) then begin
            ...
            str = str+')'
            print, 'RDPFITS: Image array ',str, '  Type=', typstrs[type]
        endif
	...
	if xunsigned then begin
	    fxaddpar,header, 'BZERO', 0, 'Updated by RDPFITS v'+mrd_version()
	endif
...
pro mrd_scale, type, scales, offsets, table, header,  $
    ...
    if type ne 0 then begin
        ...
        tabx = rdp_struct(fnames, fvalues, nrec, structyp=structyp, $
          tempdir=tempdir, silent=silent, old_struct=old_struct)
    ...
    ; Now process images and random groups.
        ...
        if sz[sz[0]+1] ne 8 then begin
            ...
            s2 = s2+')'
            tabx = rdp_struct(['params', 'array'],[s1,s2],ngr, $
                              tempdir=tempdir, silent=silent)
...
pro mrd_fixcolumn, vtype, array, heap, off, siz
    ...
    for j=0, nw-1 do begin
        ...
        case vtype of
            ...
            'A': array[0:siz[j]-1,w[j]] = byte(heap,off[j],siz[j]) 
                ;array[w] = string(heap[off:off+siz]) 
    ...
    ; Finish processing string arrays:
    ;if vtype eq 'A' then begin
	;    array = string(array)
    ;endif
...
pro mrd_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
   ...
   old_struct=old_struct, pointer_var=pointer_var, fixed_var=fixed_var
    ...
    if heapoff ne 0 and heapoff lt sz then begin 
        print, 'RDPFITS: ERROR Heap begins within data area' 
        status = -1 
        return 
    endif
    ...
    if delete then begin
        ...
        if ww[0] ne -1 then  begin
            w[vcols[ww]] = 0
            if not keyword_set(silent) then begin
                print, 'RDPFITS: ', strcompress(string(n_elements(ww))),  $
        ...
        if (wx[0] eq -1) then begin
            if not keyword_set(silent) then begin
                print, 'RDPFITS: All columns have been deleted'
            endif
	    table = 0
	    return
        endif
    ...
    if not keyword_set(pointer_var) then begin
        ...
        if n_elements(lencols) gt 0 then begin
            if not keyword_set(silent) then begin
                print, 'RDPFITS: ', strcompress(string(n_elements(lencols))), $
    ...
    if not scaling and not keyword_set(columns) then begin
        tablex = rdp_struct(fnames, fvalues, n_elements(table), structyp=structyp, $
                            tempdir=tempdir, silent=silent, old_struct=old_struct)
    endif else begin
        tablex = rdp_struct(fnames, fvalues, n_elements(table), tempdir=tempdir, $
                            silent=silent, old_struct=old_struct)
    endelse
...
pro mrd_tdim, header, index, flen, arrstr, no_tdim=no_tdim
    ...
    if strmid(tdstr,0,1) ne '(' and strmid(tdstr,len-1,1) ne ')' or len lt 3 then begin
        print, 'RDPFITS: Error: invalid TDIM for column', index
        return
    endif
    ...
    if prod ne flen then begin
        print,'RDPFITS: Error: TDIM/TFORM dimension mismatch'
        return
    endif
...
pro mrd_table, header, structyp, use_colnum,           $ 
    ...
    if xten ne 'BINTABLE' and xten ne 'A3DTABLE' then begin 
        print, 'RDPFITS: ERROR - Header is not from binary table.' 
        nfld = 0 & status = -1 
        return 
    endif 
    ...
    for i=0, nfld-1 do begin
        ...
        if ftype eq 'P' then begin 
            ...
            if dim ne 0  and dim ne 1 then begin 
                print, 'RDPFITS: Invalid dimension for variable array column '+string(i+1) 
                status = -1 
                return 
            endif
            ...
            if strpos('LXBIJKAEDCM', vf) eq -1 then begin 
                print, 'RDPFITS: Invalid type for variable array column '+string(i+1) 
                status = -1 
                return 
            endif 
        ...
        for j=0, n_elements(types) - 1 do begin
            ...
            if ftype eq types[j] then begin
                ...
                if xunsigned then begin
                    fxaddpar, header, 'TZERO'+istr, 0, 'Modified by RDPFITS V'+mrd_version()
                endif
  ...
  next_col: 
    ...
    if N_zero GT 0 then begin
        ...
        if N_zero Eq nfld then begin
            print,'RDPFITS: Error - All fields have zero length'
            return
        endif
	
        for i=0, N_zero-1 do begin
	    print,'RDPFITS: Table column ' + fnames[zero[i]] + ' has zero length'
	endfor
    ...
    if n_elements(vcls) eq 0  and  (not scaling) and not keyword_set(columns) then begin
        ...
        table = rdp_struct(fnames, fvalues, nrow, structyp=structyp, $
                           tempdir=tempdir, silent=silent, old_struct=old_struct)
	
    endif else begin
	
        table = rdp_struct(fnames, fvalues, nrow, tempdir=tempdir, $
          silent=silent, old_struct=old_struct)
	
    endelse

    if not keyword_set(silent) then begin
        print, 'RDPFITS: Binary table. ',strcompress(string(nfld)), ' columns by ',  $
          strcompress(string(nrow)), ' rows.'
        if n_elements(vcls) gt 0 then begin
                print, 'RDPFITS: Uses variable length arrays'
        endif
    endif
...
function RDPFITS, file, extension, header,      $
        structyp = structyp,                    $
    ...
    ;   Let user know version if RDPFITS being used.
    if keyword_set(version) then begin
        print,'RDPFITS: Version '+mrd_version()+' Mar. 30, 2005'
    endif
    ...
    if n_params() le 0  or n_params() gt 3 then begin
        ...
        print, 'RDPFITS: Usage'
        print, '   a=RDPFITS(file/unit, [extension, header], /version       $'
    ...
    if keyword_set(structyp) then begin
        ...
        if sz[1] ne 7 then begin
            print, 'RDPFITS: stucture type must be a string'
            return, 0
        endif
    endif
    ...
    sz = size(file)
    if (sz[0] ne 0) then begin
        print, 'RDPFITS: Vector input not supported'
        return, 0
    endif
    ...
    if sz[1] gt 0 and sz[1] lt 4 then begin
        ...
        if unit lt 0 then begin
            print, 'RDPFITS: File access error'
            status = -1
            return, 0
        endif
    endelse
    ...
    if eof(unit) then begin
        print,'RDPFITS: Extension past EOF'
    ...
    if status lt 0 then begin
        print, 'RDPFITS: Unable to read header for extension'
        if inputUnit eq 0 then begin
            free_lun, unit
        endif
        return, 0
    ...
    if xten eq '0' or xten eq 'IMAGE' then begin
        ...
        print, 'RDPFITS: Unable to process extension type:', xten
        if inputUnit eq 0 then begin
            free_lun,unit
        endif
        status = -1
