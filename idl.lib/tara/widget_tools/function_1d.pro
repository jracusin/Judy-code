...
;========================================================================
;;;
;;; Function_1D Widget: $Id: function_1d.pro 3496 2009-07-21 15:47:07Z patb $
;;;
...
FUNCTION CreateFunction1d, PARENT_WIDGET=parent, GROUP=group, BLOCK=block, $
			  WIDGET_TITLE=widget_title, PLOT_WINDOW_OPTIONS=plot_window_options
...
if keyword_set(plot_window_options) then begin
  dum = execute('plot_window, pw_id,'+plot_window_options)
endif
 ...
 ;-------------------------------------------------------------------------
  ...
  if (lstyle NE 0) then begin
        ...
        legend, plotsets.description, PSYM=psym,  $
                ...
                COLORS=colors, TEXTCOLORS=colors, CHARSIZE=0.75*!P.CHARSIZE
  endif
  end
    ...
    2: $ ; SAVE FUNCTION as FITS
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         sxaddpar, pheader, "CREATOR", "$RCSfile$, Version $Revision: 3496 $"
	 ...
	 writefits, pathname, no_data, pheader
         ...
         sxaddpar, fits_header, "CREATOR", "$RCSfile$, Version $Revision: 3496 $"
