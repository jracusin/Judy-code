...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  IF keyword_set(shift2r) AND clr NE 2 THEN BEGIN 
      ...
      read_astrans, $
        pstruct[index].run, pstruct[index].rerun, pstruct[index].camcol, $
        clr, trans, node, inc, /silent
              
      read_astrans, $
        pstruct[index].run, pstruct[index].rerun, pstruct[index].camcol, $
        2, rtrans, rnode, rinc, /silent
...
;  objy=objy-1.
  ...
  ;; maxsize. Throw away parents
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

  max_cdist = maxsize[0]
  max_rdist = maxsize[1]

  useind = where( (rcolc LE (col[1]+max_cdist) ) AND $
  ...
  IF (count NE 0) THEN BEGIN 
      ...
      FOR i=0, count-1 DO BEGIN 
          ...
          get_atlas, pstruct, gi, dir=dir, clr=clr, imtot=im, maxsize=maxsize,$
            /nodisplay,/noprompt,/silent, $
            col0=col0, row0=row0, dcol=dcol, drow=drow
...
;              im = long(im)
              ...
              ;; correct saturation
;              wsat = where(im LT 0, nsat)
;              IF nsat NE 0 THEN BEGIN 
                  ;;print,'Fixing saturation'
;                  im[wsat] = long(32767) + (im[wsat] - (-32767) )
;                  im[wsat] = 32767
;              ENDIF 
...
;  IF nl NE 0 THEN fchart[l] = 32767
  ...
  wkeep = where(keep,nw)
  IF nw EQ -1 THEN useind=-1 ELSE useind = useind[ wkeep ]
