...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; display atlas images ;;;;;;;;;;;;;;;;;;;;;;
  ...
  IF (not keyword_set(nodisplay) ) then begin 
      ...
      if keyword_set(wsize) then if (!d.x_size ne wsize(0) and $
                                     !d.y_size ne wsize(1)) $
        then window,xsize=wsize(0),ysize=wsize(1)
      ...
      if ((size(gchildren))(0) eq 0 and (size(children))(0) ne 0) then begin
          ...
          get_atlas,pstruct,index,imtot=im,dir=dir,clr=clr,silent=1,$
            noprompt=noprompt,ytitle='Input obj', $
            maguse=maguse,$
            hideradec=hideradec,maxsize=maxsize,_extra=extra
          get_atlas,pstruct,children[pchild],imtot=im,dir=dir,clr=clr,$
            silent=1,$
            maguse=maguse,$
            noprompt=noprompt, maxsize=maxsize,hideradec=hideradec,_extra=extra
...
;watch out for negative default values and remember that you may have sorted
      ...
      if ((size(gchildren))(0) ne 0) then begin
          ...
          get_atlas,pstruct,children,imtot=im,dir=dir,clr=clr,silent=1,$
            noprompt=noprompt, maxsize=maxsize,ytitle='Faint of input', $
            maguse=maguse,$
            hideradec=hideradec,_extra=extra
          get_atlas,pstruct,gchildren[pgchild],imtot=im,dir=dir,clr=clr,$
            silent=1,$
            maguse=maguse,$
            noprompt=noprompt, maxsize=maxsize,hideradec=hideradec,_extra=extra
      ...
      if (parent ne -1 and (size(children))(0) eq 0) then BEGIN
          ...
          get_atlas,pstruct,parent,imtot=im,dir=dir,silent=1,clr=clr,$
            maguse=maguse,$
            noprompt=noprompt, maxsize=maxsize,hideradec=hideradec,_extra=extra

          for i=0,nsib_use-1 do BEGIN
              iind = psib[i]
              IF (siblings(iind) EQ index) THEN BEGIN
                  ytitle='Input obj' 
              ENDIF else ytitle=''
              get_atlas,pstruct,siblings(iind),imtot=im,dir=dir,clr=clr,$
                silent=1,$
                maguse=maguse,$
                noprompt=noprompt,ytitle=ytitle, $
                hideradec=hideradec,maxsize=maxsize,_extra=extra
          endfor
      ...
      if (parent eq -1 and (size(children))(0) eq 0) then begin
          !p.multi=0
          get_atlas,pstruct,index,imtot=im,dir=dir,clr=clr,$
            silent=1,noprompt=noprompt,$
            maguse=maguse,$
            ytitle='Input obj', hideradec=hideradec,maxsize=maxsize,$
            _extra=extra
