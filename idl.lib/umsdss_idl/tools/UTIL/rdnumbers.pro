pro rdnumbers,name,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15,v16,v17, $
              v18,v19,v20,v21,v22,v23,v24,v25,$
              SKIPLINE = skipline, NUMLINE = numline,$
              DOUBLE=double, LONG=LONG, INT=INT, ULONG=ULONG
;+
; NAME:
;      RDNUMBERS
; PURPOSE:
...
;      Converted so can read other numerical types, not just
;      into float/double     Erin S. Sheldon     24-Jun-2002
;-
  On_error,2                           ;Return to caller

  if N_params() lt 2 then begin
     print,'Syntax - RDNUMBERS, name, v1, [ v2, v3,...v19 '
     print,'                    /DOUBLE, /INT, /LONG, /ULONG, $'
     print,'                    SKIPLINE =, NUMLINE = ]'
     return
  endif
...
;Create big output array and read entire file into the array
   ...
   IF keyword_set(DOUBLE) THEN BEGIN
       bigarr = dblarr(ncol, nlines, /NOZERO) 
   ENDIF ELSE IF keyword_set(LONG) THEN BEGIN 
       bigarr = lonarr(ncol, nlines, /NOZERO)
   ENDIF ELSE IF keyword_set(ULONG) THEN BEGIN 
       bigarr = ulonarr(ncol, nlines,/NOZERO)
   ENDIF ELSE BEGIN 
       bigarr = fltarr(ncol, nlines, /NOZERO) 
   ENDELSE 
   ...
   if Nvector GT 18 then v19 = reform( bigarr[18,*]) ELSE return 
   if Nvector GT 19 then v20 = reform( bigarr[19,*]) ELSE return 
   if Nvector GT 20 then v21 = reform( bigarr[20,*]) ELSE return 
   if Nvector GT 21 then v22 = reform( bigarr[21,*]) ELSE return 
   if Nvector GT 22 then v23 = reform( bigarr[22,*]) ELSE return 
   if Nvector GT 23 then v24 = reform( bigarr[23,*]) ELSE return 
   if Nvector GT 24 then v25 = reform( bigarr[24,*]) ELSE return 
  return
  end
