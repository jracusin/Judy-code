...
;              13       0.28 
;
;         ra1,dec1: list of ra's and dec's MUST BE DOUBLE PRECISON
;         ra2,dec2: ""
;         srad: search radius for each object in list 1 in radians
;               MUST BE DOUBLE PRECISON. If all search radii will be 
...
PRO htm_find_neighbors, depth,ra1,dec1,ra2,dec2,srad,ind1,ind2,distance,$
  ...
  FOR index=0L, nlist1-1 DO BEGIN 
      ...
      cenra = ra1[index]
      cendec = dec1[index]
      IF eachrad THEN searchrad = double(srad[index]) ELSE searchrad=double(srad)
  ...
  ENDIF ELSE BEGIN
      ...
      IF keyword_set(convert) THEN BEGIN 
          htm_bin2fits, frontfile, outfile, remove=remove, output_dist=output_dist
      ENDIF 
  ENDELSE 
  print
