...
pro runstatus_select, flag_struct, select_index, nkeep, input_index=input_index
  ...
  if n_params() LT 2 then begin
      print,'-syntax runstatus_select, flag_struct, select_index, nkeep, input_index=input_index'
      return
  endif

  sdssidl_setup,/silent
  IF NOT !sdssidl_def.run_status_defined THEN BEGIN 
      message,'!RUN_STATUS variable not set by sdssidl_setup'
  ENDIF 


  IF n_elements(input_index) NE 0 THEN BEGIN 
      dt = datatype(input_index)
      IF (dt EQ 'INT') OR (dt EQ 'LON') THEN BEGIN 
