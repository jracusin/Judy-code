...
;
; PURPOSE:
;    Reads in fields froma column of SDSS data into one big structure or
;    array of pointers to object structures. 
;    A rewritten version of read_photo_col that reads the newer individually 
;    wrapped tsObj files 	
;
;    Notes:
;      You do not need to put run,rerun,camcol,field,id in your taglist.  
;         These are added in automatically by read_tsobj as the required tags.
...
;               noadd=noadd, $
;               nomodrow=nomodrow, $
;               front=front, tsobjstr=tsobjstr, $
;               verbose=verbose,ex_struct=ex_struct,
;               wstring=wstring,$
;               add_seeing=add_seeing, add_sky=add_sky, status=status
;
; 
; INPUTS:  
;    directory/[run,rerun,camcol]: Either the full directory where the photo output 
;         fits files (tsObj files) live or an int or long array containing 
;         [run,rerun,camcol] in that order.
;
; OUTPUTS: 
;    struct: The structure containing the requested tags, or an array of 
;            pointers to such object pointers.
;
; OPTIONAL INPUTS:
;    -SPECIFYING THE RANGE: (default is to read first field)
...
;                     "nframes", and "last"
;
;   -TAGS TO READ: (Default is all tags. run,rerun,camcol,field,id will be
;                  added to any other taglist)
...
;                     be returned in taglist. THIS IS NOW THE DEFAULT. This
;                     keyword is now quietly ignored.
;          /corrected: read corrected files instead of tsObj files. Sets front
;                      equal to 'adatc' and look in the !sdss_shapecorr_dir if
...
;              ----- THIS KEYWORD IS NOW THE DEFAULT quietly ignored
;
...
;                    If not entered, this structure will be set and returned in
;                    through the keyword.  This can then be fed into program to
;                    save time.
;                    Contains: (1) good tags from first call
;                              (2) indices of those tags
;                              (3) the tsObj structure used. Saves time because
;                                  don't have to read in a field to establish
;                                  structure definition.
;                              (4) the file list. Avoides using fetch_file_list
;                                  which is a slow program.
;
...
;		      executed to make a cut before it concatonates into the
...
;		 	and add this infomrmation ,default tags are
...
;
;         /zero:        Add zero poits from the tsField files in tag
...
;                        -Made "id" one of the required tags along with
;                         run,rerun,camcol,field (see read_tsobj_make_tags and
;                         read_tsobj_make_struct.pro)
;                        -Made /everytag the default.  Added /default_tags which
...
;                         memory (due to the need to make a copy of
;                         the data) other than writing the data to file first,
;                         but this would be quite slow.
;                                --Erin Sheldon
...
PRO read_tsobj, dir_in, struct, $
  ...
  IF n_params() EQ 0 THEN BEGIN 
      print,'-syntax read_tsobj, directory/[run,rerun,camcol], struct, $'
      print,'            start=start, nframes=nframes, all=all, $'
      print,'            taglist=taglist, phototags=phototags, /default_tags,$'
      print,'            addrowc=addrowc, noadd=noadd, $'
      print,'            nomodrow=nomodrow, $'
      ...
      print,'            spec_cat=spec_cat,zero=zero,addspec=addspec'
      print,''
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ENDIF ELSE IF dir_type EQ 'STR' THEN BEGIN 
      ...
      ENDIF 
  ENDIF ELSE BEGIN 
      message,'First input directory/[run,rerun,camcol] must be either a scalar string or a int/long array of 3 numbers'
  ENDELSE 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  IF n_elements(tsobjstr) NE 0 THEN BEGIN 
      IF datatype(tsobjstr,/tname) EQ 'STRUCT' THEN BEGIN 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ENDIF ELSE BEGIN
      ...
      IF n_elements(start) NE 0 THEN startf=start ELSE startf = tsobjstr.fieldmin
      IF n_elements(last) NE 0 THEN lastf=last ELSE lastf = nframes + startf - 1 
      ...
      ENDIF ELSE BEGIN 
          wff=where(tsobjstr.fnums GE startf AND tsobjstr.fnums LE lastf,nfields)
      ENDELSE 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  IF makeall THEN BEGIN 
      ...
      tsobjstr = define_tsobjstr(dir, fieldmin, fieldmax, run, rerun, camcol, $
  ...
  ENDIF ELSE BEGIN
      ...
      IF findinfo THEN BEGIN 
          tsnew = define_tsobjstr(dir, fieldmin, fieldmax, run, rerun, camcol, $
                                  allfiles, allfnums, $
