...
;       /trim_bbox: trim the bounding box on encapsulated ps files.
;
; OUTPUTS:
;
;	None.
...
;      Added /trim_bbox: 13-Oct-2003
;
;-

PRO Endplot, doprint=doprint, noprint=noprint, p=p, shell=shell, help=help, setup=setup, landfix=landfix, trim_bbox=trim_bbox
...
;; Procedure to close out and print current or named plot file
   ...
   IF keyword_set(trim_bbox) THEN BEGIN 
       ...
       tf = tmpfile(prefix='/tmp/blah',suffix='.ps')
       command = 'epstool -b -c '+plotfile+' -o'+tf
       spawn,command,ans,err
       spawn,'cp -f '+tf+' '+plotfile+'; rm -f '+tf
