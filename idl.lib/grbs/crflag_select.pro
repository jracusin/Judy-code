...
;       
; PURPOSE:  
;    Makes cuts based on closure relation flags using input closure relations structure.
;      These cuts are "anded" together, they must all be true for object to
;      survive.
;    
;
; CALLING SEQUENCE:
;     crflag_select,pstruct,crflag_struct,select_index
;    
;
; INPUTS: 
;     pstruct:  A CR structure
;     crflag_struct:  premade CR flag structure.  This will require
;          any flags set to 'Y' and insist that any flag set to 'N' be off
;
; OUTPUTS: 
;    select_index:  indices of selected objects
;
; REVISION HISTORY:  JLR modified for CR stuff
;                    Judith Racusin 6/15/00, modified from flag_select by Tim Mckay
...
PRO crflag_select,pstruct,crflag_struct,select_index


  on_error, 2

  IF n_params() LT 3 THEN BEGIN
      print,'Syntax:  crflag_select,pstruct,crflag_struct,select_index'
      return
  END
