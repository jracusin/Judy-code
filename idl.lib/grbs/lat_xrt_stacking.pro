...
pro read_opt_seds
  readcol,'~/Fermi/LAT_lim_XRT/uvot/LC_for_Judy_2013/GRB_properties.txt',grb,z,d,av,a1600,t90,ebv,ebvhost,ebvhost_poserr,ebvhost_negerr,rv,a1600host,a1600host_poserr,a1600host_negerr,beta,beta_poserr,beta_negerr,mo
...
pro fit_uvot
  ...
  cd,'~/Fermi/LAT_lim_XRT/uvot/LC_for_Judy_2013'

  ;;; setting up file structure
  grbs0=file_search('GRB0*txt')
  grbs1=file_search('GRB1*txt')
  if n_elements(grbs0) gt 0 and n_elements(grbs1) gt 0 then grbs=[grbs0,grbs1]
  if n_elements(grbs0) gt 0 and n_elements(grbs1) eq 0 then grbs=grb0
  if n_elements(grbs1) gt 0 and n_elements(grbs0) eq 0 then grbs=grb1
  ngrbs=n_elements(grbs)
  
  if ngrbs gt 0 then begin 
     for i=0,ngrbs-1 do begin
        grbsplit=strsplit(grbs[i],'.',/ex)
        grbname=grbsplit[0]
        if not exist(grbname) then begin
           spawn,'mkdir '+grbname
           spawn,'mv '+grbs[i]+' '+grbname
        endif 
     endfor 
  endif 
  ...
  grbs=grbs[0:n_elements(grbs)-2]
  ngrbs=n_elements(grbs)

  for i=0,ngrbs-1 do begin
     cd,grbs[i]
     fit_lc,phil=phil,/qdp,name=grbs[i],/nohard,/flux,/uvot,file=grbs[i]+'.txt'
     cd,'..'
  endfor 
...
pro plot_uvotz
  ...
  begplot,name='~/Fermi/LAT_lim_XRT/uvot_z.eps',/land,/color,font='helvetica'
  !p.charsize=2.
...
pro plot_samp
  ...
  t90=gstr[w].t90
  begplot,name='~/Fermi/LAT_lim_XRT/t90.eps',/land,/encap,/color,font='helvetica'
  plothist,alog10(t90),bin=0.1,xtickname=['0.1','1','10','100'],xtitle='BAT T!L90!N (s)',ytitle='N',charsize=2
  oplot,alog10([2.,2.]),[0,20],line=2,color=!red
  xyouts,alog10(0.3),5,'short',charsize=2
  xyouts,alog10(5.),15,'long',charsize=2
  endplot
  ;;; bat_fluence
  w=where(gstr.bat_fluence ne 0)
  f=gstr[w].bat_fluence
  begplot,name='~/Fermi/LAT_lim_XRT/bat_fluence.eps',/land,/encap,/color,font='helvetica'
...
;;; sum of fe1[j,*]*0.1=flux2
     ...
     for j=0,noe-1 do begin 
        ...
        s=sort(flux)
        no0=where(flux ne 0)
;        fe[0,j]=10^mean(alog10(flux[no0]))
;        fe[0,j]=mean(flux[no0])
        fe[0,j]=median(flux)
        ...
        if nw2 gt 0 then begin 
           ...
           s=sort(flux)
           no0=where(flux ne 0)
;           fe[3,j]=10^mean(alog10(flux[no0]))
;           fe[3,j]=mean(flux[no0])
           fe[3,j]=median(flux)
...
;     oplot,[1.73,1.73]/h,[f1,f2]*[1.73,1.73],color=!green
     ...
     oplot,eng,fe[0,*]*eng^2
     oplot,eng,fe[1,*]*eng^2,line=2
     oplot,eng,fe[2,*]*eng^2,line=2

     writecol,'~/Fermi/LAT_lim_XRT/'+splits+'_sed.dat',eng,fe[0,*],fe[1,*],fe[2,*],fe[3,*],fe[4,*],fe[5,*],header='E(keV)   nufnu       err_low     err_high       nufnu2     err_low2   err_high2'
     if nw2 gt 0 then begin 
;        oplot,[0.3,10]/h,replicate(median(gstr[w2].flux2),2)*[1.,1.],color=!magenta
     
        oplot,eng,fe[3,*]*eng^2,color=!red
        oplot,eng,fe[4,*]*eng^2,line=2,color=!red
        oplot,eng,fe[5,*]*eng^2,line=2,color=!red
     endif 
...
;  n10000=intarr(n)
  for i=0,n-1 do begin
     ...
     wl=where(burst[i].ltstart gt 0,nwl)
...
;     endif 
  ...
  plot,lindgen(1e4),t,ytitle='Number of BAT GRBs',xtitle='Time since BAT Trigger (s)',yrange=[0,n_elements(burst)],/ysty,/xlog,xrange=[1,1e4],charsize=2,psym=10
...
;;; TO ADD TO LIST
;;; UPDATE SWIFT_TARGID.TXT & SWIFT_TRIGGER.TXT
;;; CHANGE INDEXING NUMBER IN ASSEMBLE_GRB_LIST
...
;  readcol,'~/Fermi/LAT_lim_XRT/Swift_triggers.txt',met,tid,format='(l,l)'
  ...
  targid=targid[403:*]
  trig=trig[403:*]
  xrtdet=strtrim(xrtdet[403:*],2)
  uvotdet=strtrim(uvotdet[403:*],2)
  t90=t90[403:*]
  fluence=fluence[403:*]
  xflare=xflare[403:*]
  radiodet=radiodet[403:*]
  irdet=irdet[403:*]
  optdet=optdet[403:*]
...
;  badgrbtid=tid[m2[w]]
  ...
  grb=strtrim(grb,2)
  w=where((xrtdet eq 'Y') and (grb ne 'GRB081211A' and grb ne 'GRB090510' and $
                               grb ne 'GRB100724A' and grb ne 'GRB100728A' and $
                               grb ne 'GRB110625A' and grb ne 'GRB110731A' and $
                               grb ne 'GRB120624B' and grb ne 'GRB121011A' and $
                               grb ne 'GRB130206A' and grb ne 'GRB130305A' and $
                               targid gt 100000)) ; or xrtdet eq 'P'))

  ;; 090510, 100728a 110625a, 110731a detected
  ;; 081211a, 100724a weren't observed 
  x=where(xrtdet ne 'Y')
...
;  w=where(tstart lt 1d4 and tstart ne 0)
  ...
  for i=0,n-1 do begin
     ...
     if not exist(file) then begin 
        file='~/GRBs/'+grb[i]+'A/lc_newout_phil.txt'
        adda='A'
     endif else adda=''
     if exist(file) then begin 
        lc=lcout2fits(file)
     ...
     lcfitfile='~/GRBs/'+grb[i]+adda+'/lc_fit_out_idl_int8.dat'
     if not exist(lcfitfile) then lcfitfile='~/GRBs/'+grb[i]+adda+'/lc_fit_out_idl_int7.dat'
     if exist(lcfitfile) then begin 
        read_lcfit,lcfitfile,pnames
        if pnames[0] eq 'nofit' then nofit[i]=0
        wf=where(strtrim(pnames,2) eq 'gnorm1',nwf)
        if nwf eq 1 then flares[i]='Y' else flares[i]='N'
;     if exist('~/GRBs/'+grb[i]+'/flares_gtis.dat') then flares[i]='Y' else flares[i]='N'
        trigtime[i]=date2met(trig[i],/fermi)
     endif 
  endfor 
  w=where(tstart lt 1d4 and tstart ne 0 and nofit eq 1)
...
;  endfor 
  ...
  burst=create_struct('GRB','','targid',0L,'trigtime',0d,'xtstart',0d,$
                      ...
                      'xph_ind',0d,'xph_inderr',dblarr(2),$
                      'cfratio',0d,'model','',$
                      'ltstart',dblarr(10),'ltstop',dblarr(10),$
                      'flux1_gti',d11,'flux1_gti_err',d11,$
                      'flux2_gti',d11,'flux2_gti_err',d11,$
                      'flux3_gti',d11,'flux3_gti_err',d11,$
                      'flux1',0d,'flux1err',0d,$
                      'flux2',0d,'flux2err',0d,$
                      'flux3',0d,'flux3err',0d,$
                      'extrapolation',intarr(10),$
                      'oflux1_gti',d11,'oflux1_gti_err',d11,$
                      'oflux1',0d,'oflux1err',0d)

;;flux1 - the flux using the underlying (broken) power law
;;flux 2 - the flux excluding the steep decay phase, extrapolation the plateau backwards
;;flux 3 - the flux using the full fit including flares
  ...
  burst.trigtime=trigtime*1d
  
  mwrfits,burst,'~/Fermi/LAT_lim_XRT/grb_flux_struct0.fits',/create
stop
return
end 

pro lat_xrt_stacking,ps=ps

  !x.margin=[10,3]
  !p.charsize=2
  burst=mrdfits('~/Fermi/LAT_lim_XRT/grb_flux_struct0.fits',1)
;  burst=burst[0:187]
  if burst[0].ltstart[0] eq 0 then begin 

  ;;; gather LAT time coverage intervals (up to 5 GTIs)
     for i=1,n_elements(burst) do begin
;        lat=mrdfits('~/Fermi/LAT_lim_XRT/LAT_gtis.fits',i,hdr)
;        tid=sxpar(hdr,'EXTNAME')
        lat=mrdfits('~/Fermi/LAT_lim_XRT/gti_lists/gti_list_130409.fits',i,hdr,status=status)
        if status eq 0 then begin 
           extname=sxpar(hdr,'EXTNAME')
           sep=str_sep(extname,'_')
           tid=sep[1]
           w=where(burst.targid eq tid,nw)
           print,i,burst[w].grb,burst[w].targid,'   ',tid
...
;  stop
  ...
  count=0
  cd,'~/GRBs/'
  udir='~/Fermi/LAT_lim_XRT/uvot/LC_for_Judy_2013/'
...
;     print,gdir
     ...
     burst[i].xtstart=lc[0].tstart
     lcfile='lc_fit_out_idl_int8.dat'
     if not exist(lcfile) then $
        lcfile='lc_fit_out_idl_int7.dat'
;     ofile=udir+gdir+'/'+gdir+'.txt'
;     if exist(ofile) then begin 
;        olc=lcout2fits(ofile,/uvot)
;        olcfile='lc_fit_out_idl_int7.dat'
;        useuvot=1
;     endif else useuvot=0
     flares=0
     read_lcfit,lcfile,pname,p,flares=flares
     if pname[0] ne 'nofit' then begin 
        ...
        if exist(sfile) then begin 
           ...
           if n_elements(flares) gt 0 then begin 
              sf=size(flares)
              if sf[0] ne 0 then begin 
                 nf=n_elements(flares)
                 if nf gt 0 then add='gauss'+ntostr(nf)+'_'
                 np=n_elements(p)-nf*3
                 burst[i].xflare='Y'  ;;; need to rerun with flares
              endif
           endif 
           xstop=max(lc.tstop)
           xstart=min(lc.tstart)
           ...
           ts=[(findgen(99))+1,findgen(99)*10.+100.,findgen(199)*500.+1.1e3];+burst[i].xtstart
...
;              f=intbknpow(t,p)
                 ...
                 p2=[n2,p[3:*]]
                 if p[1] gt p[3] then f2=pow(ts,p2) ;f2=intpow(t,p2)
                 mo=add+'bknpow'
              end 
              6: begin 
                 adjust_t,t,[p[2],p[4]]
;              f=intbkn2pow(t,p)
                 ...
                 p2=[n2,p[3:*]]
                 if p[1] gt p[3] then f2=bknpow(ts,p2) ;f2=intbknpow(t,p2)
                 mo=add+'bkn2pow'
              end 
              8: begin
...
;              f=intbkn3pow(t,p)
                 ...
                 p2=[n2,p[3:*]]
                 if p[1] gt p[3] then f2=bkn2pow(ts,p2) ;f2=intbkn2pow(t,p2)
                 mo=add+'bkn3pow'
              end 
              10: begin 
...
;              f=intbkn4pow(t,p)
                 ...
                 p2=[n2,p[3:*]]
                 if p[1] gt p[3] then f2=bkn3pow(ts,p2) ;f2=intbkn3pow(t,p2)
                 mo=add+'bkn4pow'
              end 
           endcase 
           ni=n_elements(t[0,*])
           ...
           print,f
           print,f2
           tmp=execute('f3='+mo+'(ts,p)')
           print,f3
           tbin=t[1,*]-t[0,*]
           nz=n_elements(t[0,*])
           burst[i].model=mo
;           nz=10
           fs=dblarr(nz) & fs2=fs & fs3=fs
           for z=0,nz-1 do begin 
              zw=where(ts ge t[0,z] and ts lt t[1,z],nzw)
              if nzw gt 1 then begin 
                 burst[i].flux1_gti[z]=int_tabulated(ts[zw],f[zw])/tbin[z]
                 if f2[0] ne 0 then burst[i].flux2_gti[z]=int_tabulated(ts[zw],f2[zw])/tbin[z]
                 burst[i].flux3_gti[z]=int_tabulated(ts[zw],f3[zw])/tbin[z]
                 if t[1,z] gt xstop then burst[i].extrapolation[z]=1
                 if t[0,z] lt xstart then burst[i].extrapolation[z]=1
              endif 
              if nzw eq 1 then begin
                 burst[i].flux1_gti[z]=f[zw]
                 burst[i].flux2_gti[z]=f2[zw]
                 burst[i].flux3_gti[z]=f3[zw]
              endif 
;;; should return XRT average flux in each LAT window, so that we can
;;; more easily split on times and maybe a switch for extrapolation or not
           endfor 
           f=total(burst[i].flux1_gti*tbin)/total(tbin)
           f2=total(burst[i].flux2_gti*tbin)/total(tbin)
           f3=total(burst[i].flux3_gti*tbin)/total(tbin)

;           f=total(f*tbin)/total(tbin)
;           f2=total(f2*tbin)/total(tbin)
;           f3=total(f3*tbin)/total(tbin)
;   if i eq 181 then stop        
;;; need to get function over small time bins and integrate
;;; (int_tabulated) over each gti, then do a weighted average
;;; int_tabulated(ts[w],f1[w])/(t[1,0]-t[0,0])

;        f=10^mean(alog10(f))
;        f2=10^mean(alog10(f2))
;        f3=10^mean(alog10(f3))
           print,f2
...
;        if f2[0] eq 0. then f2=f
           ...
           plot,[10,1d5],[1d-14,1-8],/nodata,/xlog,/ylog,xrange=[1,1e5],title=burst[i].grb,/xsty,/ysty,yrange=[1d-14,1d-5],xtitle='Time since BAT trigger (s)',ytitle='Flux (0.3-10 keV) (erg cm!U-2!N s!U-1!N)'
           for k=0,ni-1 do begin
              polyfill,[t[0,k],t[1,k],t[1,k],t[0,k],t[0,k]],[1d-14,1d-14,1d-5,1d-5,1d-14],color=!yellow
              oplot,[t[0,k],t[0,k]],[1d-15,1d10],line=2
              oplot,[t[1,k],t[1,k]],[1d-15,1d10],line=2
           endfor 
...
;           ts=ts[w]
              ...
              if np eq 4 then begin 
                 ...
                 if p[1] gt p[3] then oplot,ts,pow(ts,p2)*ratio[i],line=2
              endif 
           endelse 
           tmp=execute('fp='+mo+'(ts,p)')
           oplot,ts,fp*ratio[i],line=3,color=!blue

;           if f[0] lt 1d-4 then f[*]=0
;           if f2[0] lt 1d-4 then f2[*]=0
;           if f3[0] lt 1d-4 then f3[*]=0
           
           oplot,[1,1e5],[f,f]*ratio[i],line=1,color=!purple
           oplot,[1,1e5],[f2,f2]*ratio[i],line=1,color=!green
           oplot,[1,1e5],[f3,f3]*ratio[i],line=1,color=!orange
           for z=0,nz-1 do begin 
              oplot,[t[0,z],t[1,z]],replicate(burst[i].flux1_gti[z],2)*ratio[i],color=!purple
              oplot,[t[0,z],t[1,z]],replicate(burst[i].flux2_gti[z],2)*ratio[i],color=!green,thick=5
              oplot,[t[0,z],t[1,z]],replicate(burst[i].flux3_gti[z],2)*ratio[i],color=!orange,thick=5
           endfor 
           legend,['Flux No Flares','Flux No Flares/No Steep Decay','Total Flux'],textcolor=[!purple,!green,!orange],box=0,/top,/left
           print,np,f,f2,f3
...
;        endif 
           if keyword_set(ps) then endplot
           burst[i].flux1=f*ratio[i]  ;;; w/o flares
           burst[i].flux2=f2*ratio[i] ;;; w/o flares & no steep decay
           burst[i].flux3=f3*ratio[i] ;;; and w/ flares
           burst[i].flux1_gti=burst[i].flux1_gti*ratio[i]
           burst[i].flux2_gti=burst[i].flux2_gti*ratio[i]
           burst[i].flux3_gti=burst[i].flux3_gti*ratio[i]
           burst[i].xph_ind=spec[n_elements(spec)-1].phind
           burst[i].xph_inderr=spec[n_elements(spec)-1].phinderr
           burst[i].cfratio=ratio[i]
        endif  
;     endif 
     endif 
     cd,'..'
