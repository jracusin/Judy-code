...
pro make_sed_plot,ps=ps
  ...
  if keyword_set(ps) then begplot,name='~/Desktop/GRB080319B/SEDs/sed_evol.ps',/color,/land,font='helvetica'
  ...
  readcol,file,lines,delim='$',format='(a)',skip=1
  ...
  sed=create_struct('time','','pl_norm',0.,'pl_gam1',0.,'pl_gam1err',0.,$
                    ...
                    'b2pl_gam3',0.,'b2pl_gam3err',0.,'b2pl_chidof',0.)
  sed=replicate(sed,10)
  m=[5,5,5,5,5,4,4,4,4,4]
  tot=0.
  for i=0,9 do begin
     j=tot+1
     chunks=str_sep(lines[j-1],'||') 
     sed[i].time=chunks[1]
     
     chunks=str_sep(lines[j],'||') ;;PL
     g=str_sep(chunks[4],'+')
     sed[i].pl_gam1=g[0]
     tmp=execute('cdf=1.*'+chunks[9])
     sed[i].pl_chidof=cdf
     
     chunks=str_sep(lines[j+1],'||') ;;BPL
     g=str_sep(chunks[4],'+')
     sed[i].bpl_gam1=g[0]
     g=str_sep(chunks[5],'+')
     sed[i].bpl_ebk=g[0]
     ...
     sed[i].bpl_gam2=g[0]
     tmp=execute('cdf=1.*'+chunks[9])
     sed[i].bpl_chidof=cdf
     
     chunks=str_sep(lines[j+2],'||') ;;BPLF
     ...
     endelse 
     tmp=execute('cdf=1.*'+chunks[9])
     sed[i].bplf_chidof=cdf
     
     if i le 4 then begin
        chunks=str_sep(lines[j+3],'||') ;;B2PL
        g=str_sep(chunks[4],'+')
        sed[i].b2pl_gam1=g[0]
        g=str_sep(chunks[5],'+')
        sed[i].b2pl_ebk1=g[0]
...
;      endif 
  ...
  sed.pl_norm=[1.,1.,1.,1.,1.,1.,2.645e-2,1.735e-3,4.164e-4,6.804e-5]
  sed.bpl_norm=[7.99,3.69,2.19,1.27,0.86,0.31,9.796e-1,6.902e-1,2.865e-2,3.756e-4]
  sed.bplf_norm=[7.97,3.63,2.19,1.21,0.86,0.29,5.203e-1,1.414e-1,7.257e-3,1.478e-3]
  sed.b2pl_norm=[8.12,3.75,13.90,1.27,2.59,0,0,0,0,0]
  
  eng=[(findgen(100)+1)*1e-4,(findgen(100)+1)*1e-2,findgen(100)+1]
  colors=[!red,!orange,!cyan,!green,!blue,!royalblue,!magenta,!purple,!salmon,!grey50]
  
  xtickname=['10!U-4!N','10!U-3!N','10!U-2!N','10!U-1!N','1','10','100']
  ytickname=['10!U-6!N','10!U-5!N','10!U-4!N','10!U-3!N','10!U-2!N','10!U-1!N','1','10','10!U2!N','10!U3!N','10!U4!N','10!U5!N']
  xtitle='Energy (keV)'
  ytitle='E F(E) (kev ph cm!U-2!N s!U-1!N kev!U-1!N)' 
  
  plot,[1e-4,100.],[1e-6,1e5],/nodata,/xlog,/ylog,ysty=1,xticks=6,xminor=9,xtickname=xtickname,xtitle=xtitle,ytitle=ytitle,yticks=11,ytickname=ytickname,yminor=9
  for i=0,9 do begin
     chidof=[sed[i].pl_chidof,sed[i].bpl_chidof,sed[i].bplf_chidof,sed[i].b2pl_chidof]
     wno0=where(chidof ne 0.)
     minchi=min(chidof[wno0],m)
     m=wno0[m]
     print,sed[i].time,m,minchi
     case m of 
        0: begin
     ...
     endcase
     oplot,eng,eng*f,color=colors[i]

  endfor 
  legend,[sed.time],textcolor=colors,box=0,/top,/right
