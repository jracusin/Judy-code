...
pro model_onecomp_seds,ps=ps,noerr=noerr,justplot=justplot
  ...
  if keyword_set(ps) then begplot,name='sed_onecomp.eps',/color,/land,font='helvetica',/encaps,/cmyk else wset,0
  colors=[!red,!green,!blue,!cyan,!magenta,!salmon,!orange,!lightgreen,!turquoise,!royalblue,!purple]
  time=[150,250,350,720,1500,5856,1e4,3d4,6.3d4,2d5,5d5]
  ...
  legend,[times+' s'],/top,/right,textcolor=colors,box=0  
...
;           parname=['Norm1','p/2','nu_b','beta_e','xband']
           model='bkn2pow'
           par=[1e-3,0.25,9.7e14,0.75,2e18,1.,xband]
...
;           parinfo[2].fixed=1
           parinfo[3].fixed=1
           parinfo[4].limits=[1d18,1d20]
           parinfo[4].limited=[1,1]
           parinfo[5].limits=[1,2]
...
;           parname=['Norm1','p/2','nu_b','beta_e','xband']
        ...
        10: begin 
           ;;T+5e5s
           model='bkn2pow'
           par=[norm0*xbreak1^(ax2-ax1)*xbreak2^(ax3-ax2)*time[i]^(-ax3)/time[0]^(-ax1),0.24,1d13,0.75,nub,betae,xband]
           parname=['Norm1','(p-1)/2','nu_c','p/2','nu_b','beta_e','xband']
           parinfo=parinfo_struct(n_elements(parname))
           parinfo.parname=parname
           parinfo.value=par           
...
;           parinfo[2].fixed=1
     ...
     if n_elements(newp) gt 6 and not keyword_set(noerr) then begin
        nuc[i]=newp[2]
        conf_error,nu,flux,fluxerr,newp,perror0,model,perror,par=2,fixed=[1,1,0,1,1,1,1],log=[1,1,1,1,1,1,1];,/doplot
        nucerr[*,i]=perror[*,2]
        nucerr0[i]=perror0[2]
     endif 
...
;     if k eq 's' then stop
  ...
  if keyword_set(noerr) then stop
  
  nucerr[0,5]=nuc[5]-5.5e14
  nucerr[0,6]=nuc[6]-6e14
  nucerr[0,7]=nuc[7]-1e14
  nucerr[1,0]=1e17

  w=where(nuc ne 0,nw)
  if keyword_set(ps) then begin
     endplot 
     begplot,name='nuc_onecomp.eps',/land,/color,font='helvetica',/encap,/cmyk
  endif else window,1
...
;  time=time/(1.+0.937)
  ...
  plot,time[w],nuc[w],/xlog,/ylog,yrange=[1e13,1e17],psym=5,xtitle='SED Time (s)',ytitle=!tsym.nu+'!Lc!N',xmargin=[15,0]
  nerr=dblarr(nw)
