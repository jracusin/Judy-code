pro plot_wtpu_outputs,t,ctr,trigtime,tstart,tstop,dir=dir,ps=ps,bigbin=bigbin
  ...
  erase
  multiplot2,/init,[2,round(nctr/2.)]
...
pro wt_pileup,xrtdatadir,xrtoutdir,t,ctr,srcra,srcdec,bgra,bgdec,trigtime,z=z,bigbin=bigbin
  ...
  outdir='wt_pileup40_new_cal';_bigbin'
...
;        write_regfile,regfiles[i],srcx,srcy,40,20,roll=roll-90,/box,/det 
        write_regfile,regfiles[i],srcra,srcdec,80*a2p,20*a2p,roll=roll-90,/box
     endif else begin 
;        write_regfile,regfiles[i],srcx,srcy,[40,i],20,roll=roll-90,/box,/det
        write_regfile,regfiles[i],srcra,srcdec,[80,i]*a2p,20*a2p,roll=roll-90,/box
        ;;if even
...
;        oplot,[x1,x2],[y1,y2],psym=2,color=color[i]
  ...
  respfile='/bulk/pkg/caldb/data/swift/xrt/cpf/rmf/swxwt0to2s6_20010101v011.rmf'
;  respfile='/bulk/pkg/caldb/data/swift/xrt/cpf/rmf/swxwt0s6_20010101v010.rmf'
  ...
  ;;;;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
;  exclreg=[16,14,14,14,14,14,8,4,0,0]
  exclreg=[14,14,14,14,10,10,8,0,0,0]
...
;  for i=6,6 do begin
     ...
     texp=thigh[i]-tlow[i]
     if texp gt 500d then start=900d else start=1000d
...
;     make_xselect_file,i,evfile,0,tlow[i]+trigtime,thigh[i]+trigtime,'wt',bgreg,bgxselfile,bgevname,bgphaname,/bg
     print,start+texp+trigtime-(start+trigtime),start,start+texp
     make_xselect_file,i,evfile,0,start+trigtime,start+texp+trigtime,'wt',bgreg,bgxselfile,bgevname,bgphaname,/bg
...
;     excl=[19,exclreg[i]]
     for jj=0,0 do begin ;;only do already decided regions
        j=exclreg[i]
;        j=excl[jj]
        add='_'+ntostr(j)
        print,'XSELECT',j
...
;        xrtproducts='xrtproducts infile='+srcevname+' outdir=./ regionfile='+regfiles[j]+ 'stemout=DEFAULT gtifile=time_filter.flt expofile='+expomap+' arffile='+arfname+' lcfile='+lcfile
        ...
        xx='-1';'304'
        yy='-1';'301'
        xrtmkarf='xrtmkarf outfile='+arfname+' phafile='+phaname+' srcx='+xx+' srcy='+yy+' psfflag=yes clobber=yes expofile='+expomap+' > xrtmkarf.out'
        print,xrtmkarf
        spawn,xrtmkarf
