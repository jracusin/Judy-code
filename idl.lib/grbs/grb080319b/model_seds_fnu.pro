...
pro model_seds_fnu,justplot=justplot,outplot=outplot,ps=ps
...
;  plot,[1d13,1d21],[1d-9,10],/nodata,/xlog,/ylog,xtitle='Frequency (Hz)',ytitle='Flux (Jy)',/ysty,/xsty,xminor=9,yminor=9,yticks=10,xticks=8,ytickf='loglabels'
  ...
  if keyword_set(ps) then begplot,name='seds_2compjet.ps',/color,/land,font='helvetica'
  
;  z1=1.+0.937
  z1=1.
  time=[150,250,350,720,1500,5856,1e4,3d4,8d4,2d5,5d5]/z1
  times=['150','250','350','720','1500','5900','10!U4!N','3'+!tsym.times+'10!U4!N','8'+!tsym.times+'10!U4!N','2'+!tsym.times+'10!U5!N','5'+!tsym.times+'10!U5!N']
  h=4.1356692d-18
  vband=5.55e14
  xband=2./h
...
;  nucwjfs=1.36245d16
  ...
  numwjfs=3d10
  pwjfs=1.83
  norm10=1.56d-4;7.265d-5
  nucwjfs=3.99d15;1.48d15
  ...
  for i=0,n-1 do begin
     ...
     if not keyword_set(justplot) or (keyword_set(justplot) and i eq 0) and not keyword_set(outplot) then $
        plot,[1d9,1d21],[1d-9,10],/nodata,/xlog,/ylog,xtitle='Frequency (Hz)',ytitle='Flux Density (Jy)',/ysty,/xsty,xminor=9,yminor=9,yticks=10,xticks=12,ytickf='loglabels',title=title
...
;              norm3=norm10*tt10^(-ax3)
              norm3=10*tt10^(fnwjfs)
              par=[norm1,-1./3.,1e14,0.5,5d15,pwjrs/2.,1e14,$
                   norm2,-1./3.,9d16,0.8,2d18,1.1,1e17,$
                   norm3,-1./3.,numwjfs*tt10^(-s32),(pwjfs-1.)/2.,nucwjfs*tt10^(1./2),pwjfs/2.,numwjfs*tt10^(-s32)]
...
;              norm3=norm10*tt10^(-ax3)
              norm1=norm0a*tt^(-2.)
              norm2=norm0b*tt^(-0.5)
              norm3=norm10*tt10^(fnwjfs)
              par=[norm1,-1./3.,nucrs*tt^(-1.),0.5,numrs*tt^(-1.),pwjrs/2.,nucrs*tt^(-1.),$
                   norm2,-1./3.,numnjfs*tt^(-s32),(pnj-1.)/2.,nucnjfs*tt^0.5,pnj/2.,numnjfs*tt^(-s32),$
                   norm3,-1./3.,numwjfs*tt10^(-s32),(pwjfs-1.)/2.,nucwjfs*tt10^(1./2),pwjfs/2.,numwjfs*tt10^(-s32)]
              model='bkn2pow_bkn2pow_bkn2pow'
              parinfo=parinfo_struct(n_elements(par))
              parinfo.value=par
