...
pro plot_seds
  ...
  readcol,'~/Desktop/GRB130427A/NuSTAR/eeuf_Hz_2sed.dat',freq,freqerr,nfnu,nfnuerr,mnfnu,format='(a,a,a,a,a)'
  readcol,'~/Desktop/GRB130427A/NuSTAR/eeuf_noabs_Hz.dat',freq0,freqerr0,nfnu0,nfnuerr0,mnfnu0,format='(a,a,a,a,a)'
  norms=[4.43458E-03,7.26778E-04,4.31155E-03,4.28141E-03,1.05691E-03,1.10303E-03,9.34482E-04,9.35182E-04] 
  normratio=[norms[3]/norms[0],norms[7]/norms[1],norms[3]/norms[2],norms[3]/norms[3],norms[7]/norms[4],norms[7]/norms[5],norms[7]/norms[6],norms[7]/norms[7]]
...
;  tnustop=[tnu2,tnu4,tnu6]
  ...
  wi1=[w1,w3,w4]
  nwi1=[n_elements(w1),n_elements(w3),n_elements(w4)]
  wi2=[w2,w5,w6,w7,w8]
  nwi2=[n_elements(w2),n_elements(w5),n_elements(w6),n_elements(w7),n_elements(w8)]
  freq1=double(freq[wi1])
  freqerr1=double(freqerr[wi1])
  nfnu1=double(nfnu[wi1])
  nfnuerr1=double(nfnuerr[wi1])
  mnfnu1=double(mnfnu[wi1])
  normratio1=[replicate(normratio[0],nwi1[0]),replicate(normratio[2],nwi1[1]),replicate(normratio[3],nwi1[2])]
  absratio1=double(mnfnu0[wi1])/mnfnu1*normratio1
;  absratio1=normratio1
  freq2=double(freq[wi2])
  freqerr2=double(freqerr[wi2])
  nfnu2=double(nfnu[wi2])
  nfnuerr2=double(nfnuerr[wi2])
  mnfnu2=double(mnfnu[wi2])
  normratio2=[replicate(normratio[1],nwi2[0]),replicate(normratio[4],nwi2[1]),replicate(normratio[5],nwi1[2]),replicate(normratio[6],nwi2[3]),replicate(normratio[7],nwi2[4])]
  absratio2=double(mnfnu0[wi2])/mnfnu2*normratio2
;  absratio2=normratio2
...
;  !p.multi=[0,1,2]
  ...
  for z=0,1 do begin 
   ...
   if z eq 0 then begin 
      plot,eng1,nfnu1*absratio1,/xlog,/ylog,psym=3,xrange=[0.3,100e6],/xsty,/ysty,yrange=[1e-13,4e-10],ytitle=!tsym.nu+'F!L'+!tsym.nu+'!N (ergs cm!U-2!Ns!U-1!N)';,xtitle='Energy (keV)'
      ...
      color=[replicate(!lightgreen,a[0]),replicate(!forestgreen,a[1]-a[0]),replicate(!forestgreen,n_elements(eng1)-a[1])]
        for i=0,n_elements(eng1)-1 do begin
           oplot,[eng1[i]-engerr1[i],eng1[i]+engerr1[i]],[nfnu1[i],nfnu1[i]]*absratio1[i],color=color[i]
           oplot,[eng1[i],eng1[i]],[nfnu1[i]-nfnuerr1[i],nfnu1[i]+nfnuerr1[i]]*absratio1[i],color=color[i]
        endfor 
     endif else begin 
        ...
        color=[replicate(!dodgerblue,a[0]),replicate(!royalblue,a[1]-a[0]),replicate(!royalblue,a[2]-a[1]),replicate(!royalblue,a[3]-a[2]),replicate(!royalblue,n_elements(eng2)-a[3])]
        
        oplot,eng2,nfnu2*absratio2,psym=3,color=!blue
        for i=0,n_elements(eng2)-1 do begin
           oplot,[eng2[i]-engerr2[i],eng2[i]+engerr2[i]],[nfnu2[i],nfnu2[i]]*absratio2[i],color=color[i]
           oplot,[eng2[i],eng2[i]],[nfnu2[i]-nfnuerr2[i],nfnu2[i]+nfnuerr2[i]]*absratio2[i],color=color[i]
        endfor 
     endelse
...
;;   endfor 
     ...
     s2=sort(eng2)
;  s3=sort(eng3)
     eng=[(dindgen(9)+1.)/10.,(dindgen(99)+1.),(dindgen(9)+1)*1e2,(dindgen(9)+1)*1e3,(dindgen(9)+1.)*1e4,(dindgen(9)+1.)*1e5,(dindgen(9)+1.)*1e6,(dindgen(10)+1.)*1e7]
     engerr=[replicate(0.1,9),replicate(1.,99),replicate(1e3,9),replicate(1e4,9),replicate(1e5,9),replicate(1e6,9)]
...
;  p=[1.84720,1.93351,2.02466]
;  perr=[0.03,0.10,0.145]
;  flux0=[-10.5642,-11.3207,-11.4358]
;  fluxerr0=[0.021,0.070,0.099]

;  flux0=[-10.46,-11.15,-11.24]
;  fluxerr0=[0.013,0.039,0.054]
;  flux0=[-10.97,-11.62,-11.67]
;  fluxerr0=[0.011,0.034,0.048]
     ...
     norm=[norms[3],norms[7]]

     normerr=(fluxerr/flux)*norm
     
     f=pow(eng,[norm[0],p[0]])
...
;  polyfill,x,y,color=!green,/transparent
     if z eq 0 then begin 
        oplot,eng,eng^2*f*arb,color=!green
        oplot,eng,eng^2*(f+sqrt(dfdn^2*normerr[0]^2+dfdp^2*perr[0]^2))*arb,line=1,color=!green
        oplot,eng,eng^2*(f-sqrt(dfdn^2*normerr[0]^2+dfdp^2*perr[0]^2))*arb,line=1,color=!green
     endif 
     ...
     if z eq 1 then begin 
        ...
        oplot,eng,eng^2*(f+sqrt(dfdn^2*normerr[1]^2+dfdp^2*perr[1]^2))*arb,line=1,color=!blue
        oplot,eng,eng^2*(f-sqrt(dfdn^2*normerr[1]^2+dfdp^2*perr[1]^2))*arb,line=1,color=!blue
     endif 
     ;; LAT UL
     latgam=[1.85,1.92]
...
;  oplot,[eng[w],reverse(eng[w]),eng[w[0]]],[iflux1errp[0]/nfact1,reverse(iflux1errm[0]/nfact1),iflux1errp[0]/nfact1[0]],color=!blue
     ...
     pm=!tsym.plusminus
     if z eq 0 then $
        legend,['T+T0=1.03-1.61 x 10!U5!N s ('+g+'='+numdec(p[0],2)+pm+numdec(perr[0],2)+')'],box=0,/bottom,/left,textcolor=!green else $
