...
function jet_angle,tbreak,z=z,eiso=eiso,eta=eta,silent=silent,xi=xi,n=n
;;; theta j calculation from Burrows et al. (2006) (Venice proceedings)
  ;;TBREAK MUST BE IN DAYS!!!!

  if n_params() eq 0 then begin
     print,'syntax: theta_j=jet_angle(tbreak,z=z,eiso=eiso,eta=eta,silent=silent,xi=xi,n=n)'
     print,'          tbreak in days, default z=2.5'
     return,0
  end 
;  if tbreak gt 500 then tbreak=tbreak/86400d
  ...
  if n_elements(eiso) eq 0 then eiso=3.7d52;1d53
  if eiso gt 1d40 then eiso=eiso/1d53
  if n_elements(n) eq 0 then n=1d
;const=0.114
;  const=0.064
;  const=0.06162  ;;recalculate as of 11/20/08 for eta=0.2
  const=0.05651  ;;recalulated as of 12/2/08 for eta=0.1
  
  ;;changed eta=0.1 as of 11/20/08
  if n_elements(eta) eq 0 then eta=0.1

  xi=(3.5/(1.+z))^(3./8.)*(eta/0.1)^(1./8.)*(n/eiso)^(1./8.)
;print,xi,eiso
