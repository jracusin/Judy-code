pro lcout2lcsimp,file=file,outfile=outfile
  
  if n_elements(file) eq 0 then file='lc_out.txt'
  if n_elements(outfile) eq 0 then outfile='lc_simp.txt'
  ...
  writecol,'lc_simp.txt',time,timerr,cts,err,curr_ftype,header='time     timerr   cts     err  type'
