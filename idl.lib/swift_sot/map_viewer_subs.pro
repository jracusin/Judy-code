...
; ***********************************************************************
  ...
  get_lun,luin
  lut_file = '~/idl.lib/swift_sot/idl.color2.lut' 
  openr,luin,lut_file
...
;     if ((consdist1 ge econ+scon-20. and consdist1 le econ+scon+20.) or (consdist2 ge econ+mcon-20. and consdist2 le econ+mcon+20.)) or (noskip eq 1) or (strmid(k,strlen(k)-1,1) eq 'm' or strmid(k,strlen(k)-1,1) eq 's') and j gt 0 then begin 
  ...
  if select ne 0 then begin
     ...
     plat=pdec
  endelse 
  wobj1=where(abs(plon-p1[0]) lt 0.001 and abs(plat-p1[1]) lt 0.001,nwobj1)
  wobj2=where(abs(plon-p2[0]) lt 0.001 and abs(plat-p2[1]) lt 0.001,nwobj2)
...
;     radec2=' ('+ntostr(fix(ppst[wobj2[0]].ra))+','+ntostr(fix(ppst[wobj2[0]].dec))+')' 
  ...
  slewend=ppst[j].slewend
  if slewend eq 0 then slewend=met2date(date2met(ppst[j].slewbeg)+120.)
  sinfo2='Slew End:   '+slewend
...
;        euler,moonra,moondec,mlon,mlat,3
     ...
     if not gomin then begin
        predict_slew,p1[0],p1[1],p2[0],p2[1],elon,elat,slon,slat,mlon,mlat,olo,ola,tslew,/noplot,dostop=dostop,theta_roll=theta_roll,dist=dist,noslew=noslew,phi=phi
...
;        was=where(abs(olo-aslon) lt 3 and abs(ola-aslat) lt 3,nwas)
     was=where(asdist lt 15. and abs(ola-aslat) lt 5,nwas)
     if nwas gt 0 then minasdist=min(asdist[was])
...
;     printf,97,theta_roll[0]*!radeg,deltaroll[0],asdist[mas],tslew
     ...
     ;;;check slew around moon and ecliptic crossing safing events
     ;;;also checks to see if start and end point are near moon and if slewpath goes within 35 deg of anti-sun
     mdist=calc_coord_offset(olo,ola,mlon,mlat)/3600.
     minmdist=min(mdist)
     pdist=[mdist[0],mdist[n_elements(mdist)-1]]
     masdist=calc_coord_offset(olo,ola,aslon,aslat)/3600.
     minmasdist=min(masdist)
     if minmdist lt 23 and phi ne 0 and ((p1[1] gt 0 and p2[1] lt 0) or (p1[1] lt 0 and p2[1] gt 0)) and pdist[0] lt 30 and pdist[1] lt 30 and minmasdist lt 35 then moonsafe=1 else moonsafe=0
...
;     print,smomra,smomdec
     ...
     if dorb0 lt 30. and dorb1 lt 30. then nworb=1 else nworb=0
     print,'dorb = ',dorb0,dorb1
     ...
     if dmag0 lt 30. and dmag1 lt 30. then nwmag=1 else nwmag=0
     print,'dmag = ',dmag0,dmag1
...
;        d=separation(olo[0],ola[0],olo[1],ola[1])/3600.
        d=acos(cos(dla*!dtor)*cos(dlo*!dtor)+sin(dla*!dtor)*sin(dlo*!dtor)*cos(!pi/2.))*!radeg
;        if dlo lt 0 then d=-1.*d
;        stheta=acos((cos(dla*!dtor)-cos(dlo*!dtor)*cos(d*!dtor))/(sin(dlo*!dtor)*sin(d*!dtor)))*!radeg
;        stheta=asin(sin(!pi/2.)/sin(dlo*!dtor)*sin(dla*!dtor))*!radeg
        stheta=asin(sin(dla*!dtor)/sin(d*!dtor))*!radeg
;        stheta=acos(-(cos(abs(dlo)*!dtor)*cos(abs(d)*!dtor))/(sin(abs(dlo)*!dtor)*sin(d*!dtor)*cos(abs(dla)*!dtor)))*!radeg
        stheta2=atan(dla/dlo)*!radeg
;        print,olo[0],ola[0]
;        print,olo[1],ola[1]
        print,'Obj traj: ',dlo,dla,stheta,stheta2

        if dlo gt 0 and dla gt 0 then stheta=stheta
        if dlo lt 0 and dla gt 0 then stheta=stheta+180.
        if dlo lt 0 and dla lt 0 then stheta=stheta+180.
        if dlo gt 0 and dla lt 0 then stheta=stheta+360.
        print,stheta
...
;        dlon=(olo[0]-elon)*!dtor
;        dlat=dist_overpole(ola[0],elat)*!dtor
;        dlon=dist_overpole(olo[0],elon)*!dtor
;        if olo[0] lt 180 and elon gt 180 then dlon=360.-elon+olo[0]
;        if olo[0] gt 180 and elon gt 180 then dlon=abs(olo[0]-elon)
;        if elon lt 180 and olo[0] gt 180 then dlon=olo[0]-elon
;        if olo[0] lt 180 and elon lt 180 then dlon=olo[0]-elon
;        dlon=dlon*!dtor
;        print,olo[0],ola[0]
;        if (elon gt olo[0] and elon lt 180) or (elon lt olo[0] and elon lt 180) then begin
;           dlon=2*!pi-dlon
;           upaz=2.*!pi
;        endif else upaz=0.
        
;        if ola[0] lt elat then dlat=-1.*dlat
;        if olo[0] lt elon and elon lt 180 then dlon=-1.*dlon
;        if elon lt 180 and olo[0] gt 180 then dlon=-1.*dlon
;        if olo[0] lt 180 and elon gt 180 then dlon=-dlon
;        if olo[0] lt 180 and elon lt 180 and olo[0] gt elon then dlon=-dlon
        
        
;        cosaz=(cos(dlat)-cos(econ*!dtor)^2)/(sin(econ*!dtor)^2)
;        az=acos(cosaz)
;        d=acos(cos(dlat)*cos(dlon)+sin(dlat)*sin(dlon)*cos(!pi/2.))
;        dd=d
;        az=asin(sin(dlat)*sin(!pi/2.)/sin(d))
;        print,az*!radeg
;        res=map_2points(elon,elat,olo[0],ola[0])
        res=map_2points(elon,elat,elon1,elat1)
        az=res[1]*!dtor
        dd=res[0]*!dtor
        az=az+!pi/2.
;        if (elon lt 180 and olo[0] lt 180) or (elon gt 180 and olo[0] gt 180) then same=1 else same=0
        
;        if elat lt ola[0] and ((elon lt olo[0] and same) or (elon gt olo[0] and not same)) then az=!pi/2.-az
;        if elat lt ola[0] and ((elon gt olo[0] and same) or (elon lt olo[0] and not same)) then az=!pi/2.-az;az=az+3.*!pi/2.
;        if elat gt ola[0] and ((elon gt olo[0] and same) or (elon lt olo[0] and not same)) then az=az+!pi/2.;az=3.*!pi/2.-az
;        if elat gt ola[0] and ((elon lt olo[0] and same) or (elon gt olo[0] and not same)) then az=az+!pi/2
;        if upaz ne 0 then az=upaz-az
        
;        az=!pi/2.-az
;        print,az*!radeg
;        if dlon lt 0 and dlat gt 0 then az=az+!pi
;        if dlon lt 0 and dlat lt 0 then az=az+!pi
;        if dlon gt 0 and dlat lt 0 then az=az+2.*!pi
;        print,az*!radeg
;        if dlat lt 0 then az=az+!pi/2. ;;; fixes negative dlat
;        print,dlon*!radeg,dlat*!radeg,d*!radeg,'   Az=',az*!radeg,same
        
        temp1=ll_arc_distance([elon,elat]*!dtor,(econ-5.)*!dtor,az)*!radeg       
        temp2=ll_arc_distance([elon,elat]*!dtor,econ*!dtor,az)*!radeg 
;        temp1=ll_arc_distance([elon,elat]*!dtor,(econ)*!dtor,az)*!radeg       
;        temp2=ll_arc_distance([elon,elat]*!dtor,dd,az)*!radeg 
...
;        d=separation(temp1[0],temp1[1],temp2[0],temp2[1])/3600.
        d=acos(cos(dla*!dtor)*cos(dlo*!dtor)+sin(dla*!dtor)*sin(dlo*!dtor)*cos(!pi/2.))*!radeg
;        if dlo lt 0 then d=-1.*d
;        otheta=acos((cos(dla*!dtor)-cos(dlo*!dtor)*cos(d*!dtor))/(sin(dlo*!dtor)*sin(d*!dtor)))*!radeg
;        otheta=asin(sin(!pi/2.)/sin(dlo*!dtor)*sin(dla*!dtor))*!radeg
        otheta=asin(sin(dla*!dtor)/sin(d*!dtor))*!radeg
;        otheta=acos(-(cos(abs(dlo)*!dtor)*cos(abs(d)*!dtor))/(sin(abs(dlo)*!dtor)*sin(d*!dtor)*cos(abs(dla)*!dtor)))*!radeg

        otheta2=atan(dla/dlo)*!radeg
        print,'earth con: ',dlo,dla,otheta,otheta2
...
;        endelse 
;;;        if dlo gt 0 and dla gt 0 then otheta=otheta
;;;        if dlo lt 0 and dla gt 0 then otheta=otheta+90.;180.
;;;       if dlo lt 0 and dla lt 0 then otheta=otheta+180.
;;;        if dlo gt 0 and dla lt 0 then otheta=otheta+360.
;        stop
...
;     if ((nws gt 0 or nwm gt 0) and (trajdist gt 60. or keyword_set(maybebadslew))) or nwas gt 0 or noslew eq 1 and not onlyantisun or ((nworb gt 0 or nwmag gt 0) and nomd eq 1) or (nwas gt 0 and onlyantisun) or (noskip eq 1) or (strmid(k,strlen(k)-1,1) eq 'm') or strmid(k,strlen(k)-1,1) eq 's' then begin
     if ((((nws gt 0 or nwm gt 0) and (trajdist gt 60. or keyword_set(maybebadslew))) or nwas gt 0 or noslew eq 1 or ((nworb gt 0 or nwmag gt 0) and nomd eq 1) or moonsafe) and not onlyantisun and not onlymoon) or (nwas gt 0 and onlyantisun) or (noskip eq 1) or (moonsafe and onlymoon) or (strmid(k,strlen(k)-1,1) eq 'm') or strmid(k,strlen(k)-1,1) eq 's' then begin
;        print,'check_mom',nomd,nws,nwm,trajdist,noslew
;        print,'final mdist = ',fmdist
        reason=''
...
;        if nomd eq 1 then reason=[reason,'Magnetic & Orbit planes aligned']
        ...
        if moonsafe gt 0 then reason=[reason,'Slew around moon cross ecliptic']
...
;;       endfor 
           for i=1,24 do begin
              cir_2p,[xarr2[i-1],yarr2[i-1]],[xarr2[i],yarr2[i]],0.3,-1,color=!blue
           endfor 
        endfor 
        
        plots,cpointdata[0,0], cpointdata[0,1],psym=8,color=255,symsize=3 ;sun
        plots,cpointdata[1,0], cpointdata[1,1],psym=8,color=0,symsize=3   ;moon
        plots,cpointdata[2,0], cpointdata[2,1],psym=8,color=!blue,symsize=1 ;earth
        vsym,5,/star,/fill
...
;        plots,-1.*temp1[0],temp1[1],psym=2,color=!blue
;        plots,-1.*temp2[0],temp2[1],psym=2,color=!blue,symsize=2
;        for i=0,30 do begin
;           temp3=ll_arc_distance([elon,elat]*!dtor,dd/30.*i,az)*!radeg 
;           plots,-1.*temp3[0],temp3[1],psym=2
;        endfor 
        if sfpoint then begin 
           for i=1,6 do $
...
;           w=where(sctime ge date2met(ppst[j].slewbeg) and sctime le date2met(ppst[j].slewend))
           if not gomin then $
              w=where(sctime ge date2met(ppst[j].slewbeg) and sctime le date2met(ppst[j].slewbeg)+tslew[0]) else begin
              day=fix(date[1])
              tmp=(date[1]-day)*24.
              hr=fix(tmp)
              tmp1=(tmp-hr)*60.
