...
if (idebug ge 5) then stop
	...
	xyouts,pointdata(*,0), pointdata(*,1),numbers, alignment=0.5,color=100,size=2,charthick=3
...
;  jdcnv,date[0],month,day,0.,jd
;date[0]=yr
;date[1]=day of year
  ephdir='/home/xrt/EPH/'
  ephfile=ephdir+'STK_EPH_2004363_2005010_01.txt'
  openr,lun,ephfile,/get_lun
  dummy=parse_str(lun)
  earth_lnes=get_lines(ephfile)
  earth_dat=fltarr(earth_lnes,9)
  ct=0.
  while not eof(lun) do begin
      line=parse_str(lun)
...
;stop
;          if ((strmid(line(0),0,3) eq strmid(date[1],0,3))) then begin
;          if ((strmid(line(0),0,3) eq strmid(date[1],0,3)) and (line(1) eq '00:00:00.00')) then begin
          if ((strmid(line(0),0,3) eq strmid(date[1],0,3)) and (line(1) eq partdaystr)) then begin
              print,line(0),' ',line(7),' ',line(1)
;              earth_ra=line(7)+180.
;              if earth_ra gt 360. then earth_ra=earth_ra-360.
;;	      earth_ra=line(7)+90.
;;	      if earth_ra gt 0. then earth_ra=earth_ra-360.
	  if line(6) gt 0 then earth_ra=line(5)+180.
	  if line(6) lt 0 then earth_ra=line(5)+360.+180.
	  if earth_ra gt 360. then earth_ra=earth_ra-360.
;;	      phase=acos(line(8)/20.58)
;;	      earth_dec=sin(phase)*20.58
;;	      earth_dec=-1.*earth_dec      
;              earth_dec=-1.*line(8)
	  earth_dec=-1.*line(5)
          endif
      endif
  endwhile
  free_lun,lun
  earth_dat=earth_dat(0:ct-1,*)
  jdcnv,date[0],month,day,partday*24.,jd
  sunpos,jd,sunra,sundec
  moonpos,jd,moonra,moondec
; stop 
  pointdata=fltarr(2,3)
  pointdata[0,0]=sunra
  pointdata[1,0]=sundec
  pointdata[0,1]=moonra
  pointdata[1,1]=moondec
  pointdata[0,2]=earth_ra
  pointdata[1,2]=earth_dec
  num_pairs=3
...
;    wout=where(pointdata[1,*] lt -90.,nwout)
;    if nwout gt 0 then pointdata[1,wout]=pointdata[wout,1]+2.*(-90.-pointdata[1,wout])
;    wout=where(pointdata[1,*] gt 90.,nwout)
;    if nwout gt 0 then pointdata[1,wout]=pointdata[wout,1]+2.*(90.-pointdata[1,wout])
;    wout=where(pointdata[0,*] lt 0.,nwout)
;    if nwout gt 0 then pointdata[0,wout]=pointdata[0,wout]+360.
    
;    pointdata(0,*) = -1.0*pointdata(0,*) ; invert for display
    ;print,pointdata    
    
    plotsym,0,/fill,1
    plots,pointdata[0,0], pointdata[0,1],psym=8,color=255,symsize=3   ;sun
    plots,pointdata[1,0], pointdata[1,1],psym=8,color=0,symsize=3   ;moon
    plots,pointdata[2,0], pointdata[2,1],psym=8,color=128,symsize=3   ;earth
    ...
    xarr=xarr+pointdata[0,0]  &  yarr=yarr+pointdata[0,1]
    xarr2=fltarr(49)
    yarr2=fltarr(49)
    for ct=0,48 do begin
        temp=ll_arc_distance([pointdata[0,0],pointdata[0,1]]*!pi/180.,45.*!pi/180.,ct*7.5*!pi/180.)
...
;    endfor 
    ...
    psize=30.
    xarr = psize*cos(ang)  &  yarr = psize*sin(ang)
    xarr=xarr+pointdata[1,0]  &  yarr=yarr+pointdata[1,1]
    xarr2=fltarr(49)
    yarr2=fltarr(49)
    for ct=0,48 do begin
        temp=ll_arc_distance([pointdata[1,0],pointdata[1,1]]*!pi/180.,30.*!pi/180.,ct*7.5*!pi/180.)
        xarr2(ct)=temp(0)*180./!pi
        yarr2(ct)=temp(1)*180./!pi
    endfor
    for i=1,48 do begin
...
;    endfor 
    ...
    psize=100.
    xarr = psize*cos(ang)  &  yarr = psize*sin(ang)
    xarr=xarr+pointdata[2,0]  &  yarr=yarr+pointdata[2,1]
    xarr2=fltarr(49)
    yarr2=fltarr(49)
    for ct=0,48 do begin
        temp=ll_arc_distance([pointdata[2,0],pointdata[2,1]]*!pi/180.,100.*!pi/180.,ct*7.5*!pi/180.)
        xarr2(ct)=temp(0)*180./!pi
        yarr2(ct)=temp(1)*180./!pi
    endfor
    for i=1,48 do begin
