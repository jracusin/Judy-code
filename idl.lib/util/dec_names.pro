...
; Carrying
  ...
  min_name = deg_name
  sec_name = deg_name
  deg_name[0] = strcompress(string(deg[0]), /rem)+!tsym.degrees
...
;    if deg[i]-deg[i-1] ne 0 then $
      deg_name[i] =  strcompress(string(deg[i]), /rem)+!tsym.degrees
;    if minute[i]-minute[i-1] ne 0 then $
      min_name[i] = string(minute[i],  format = '(i2.2)')+a
;    if sec[i]-sec[i-1] ne 0 then $
      if sec[i] lt 10. then add='0' else add=''
      sec_name[i] = add+decimals(sec[i], decs)+b
