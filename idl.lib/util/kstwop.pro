 pro kstwop, data1, data2, D, prob, plot=plot,_extra=_extra
;+
; NAME:
;       KSTWOP
; PURPOSE:
...
;-
 ...
 if ( N_params() LT 4 ) then begin
    print,'Syntax - KSTWO, data1, data2, d, prob,/plot'
    return
 endif
...
; funtions
 ...
 D = max( abs( fn1[id1] - fn2[id2] ), msub ) 
 N_eff =  n1*n2/ float(n1 + n2)              ;Effective # of data points
 PROB_KS, D, N_eff, prob                ;Compute significance of statistic
 
 if keyword_set(plot) then begin
    allx1=[0,rebin(sortdata1,2*n1,/samp)]
    ally1=[rebin(fn1,2*n1,/samp),1.0]
    plot, allx1,ally1,_EXTRA =_extra
    allx2=[0,rebin(sortdata2,2*n2,/samp)]
    ally2=[rebin(fn2,2*n2,/samp),1.0]
    oplot,allx2,ally2,line=1
    plots,sortdata1[id1[msub]]+[0,0],[0,1]
    if prob lt 0.01 then p=sigfig(prob,3,/sci) else p=sigfig(prob,3)
    legend,['D='+sigfig(d,2),'prob='+p],/bottom,/right,box=0,charsize=1
 endif
