pro tvellipse2, rmax, rmin, xc, yc, pos_ang, color, DATA = data, THICK = thick, $
...
;-
 ...
 w=where(yprime gt 90,nw)
 g=where(yprime le 90 and yprime ge -90,ng)
 w2=where(yprime lt -90,nw2)

 if keyword_set(data) then begin
    if ng gt 0 then $
       plots, xprime[g], yprime[g], /DATA, COLOR=color, THICK=thick, LINESTYLE=linestyle
    if nw gt 0 then $
       plots, xprime[w], yprime[w]-180, /DATA, COLOR=color, THICK=thick, LINESTYLE=linestyle
    if nw2 gt 0 then $
       plots, xprime[w2], yprime[w2]+180, /DATA, COLOR=color, THICK=thick, LINESTYLE=linestyle
 endif else $
