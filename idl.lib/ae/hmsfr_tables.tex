% $Id: hmsfr_tables.tex 3511 2009-08-07 17:59:23Z psb6 $
...
\\Col.\ (16): Effective exposure time: approximate time the source would have to be observed at the aimpoint of the ACIS-I detector in Cycle ??? to obtain the reported number of net counts (see \url{http://asc.harvard.edu/cgi-bin/build_viewer.cgi?ea}).
...
\tablenotetext{b}{
All fits used the ``tbabs*vapec'' model in XSPEC abundances frozen at the values adopted by the XEST study \citep{Gudel07}, relative to  \citet{Anders89}, scaled to \citet{Wilms00}, using the tbabs absorption code in XSPEC.  
...
\tablenotetext{b}{
All fits used the ``tbabs(powerlaw)'' model in XSPEC.  
