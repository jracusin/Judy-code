; $Id: pass1_hk_viewer.pro 1.0 2002/01/18 17:25:30 burrows $
;
;==========================================================================
;;; hk Viewer Widget
...
;FUNCTION pass1_hk_viewer, image, ID=id, PARENT=parent, TITLE=title, $
...
;;; The pass1_hk_viewer widget will be realized when the parent is realized.
;;; The parent widget is responsible for asking the pass1_hk_viewer widget
;;; to display the data for the first time by calling pass1_hk_viewer with
;;; the ID keyword.  THIS IS NOT YET IMPLEMENTED!!

;;; For example:
;;;
;;;     pass1_hk_viewer = pass1_hk_viewer(image, PARENT=parent_id)
;;;     widget_control, parent_id, /REALIZE
;;;     dummy = pass1_hk_viewer( ID=pass1_hk_viewer )

;;; Second, the event handler of a realized widget can create a top level
;;; pass1_hk_viewer that will be managed independently by the XMANAGER by
;;; omitting the parameter PARENT.
;;; The parameter TITLE may be supplied to put in the window frame.
;;; The pass1_hk_viewer will realize itself, draw itself, and register with
;;; the XMANAGER.  

;;; For example:
;;;
;;;     pass1_hk_viewer = pass1_hk_viewer(image, GROUP=creator_id)

;;; Third, a pass1_hk_viewer may be created at the command line or by a regular
...
;;; For example:
;;;
;;;     pass1_hk_viewer = pass1_hk_viewer(image, TITLE='My Data')
;;;     xmanager
;==========================================================================
...
;;; The pass1_hk_viewer, p1_image_viewer, and two p1_image_viewer_roi widgets 
...
;;; 1. Same as E.
;
; $Log: pass1_hk_viewer.pro $
;

;==========================================================================
;;; This routine is called to change the image that is displayed.
;==========================================================================
PRO ModifyhkViewer, top_base, image, _EXTRA=extra
...
;; Display statistics.
hk_viewer_show_stats, state, top_base
...
;==========================================================================
PRO hk_viewer_show_stats, State, top_base
...
;==========================================================================
FUNCTION hkViewerEventFn, Event

@pass1_common

widget_control, /HOURGLASS

;; Get the pass1_hk_viewer state structure.
...
; Image viewer widget events (ROI annotations need to be redrawn).
  state.p1_image_viewer: $
   ...
   if ((marker_x ge 1) and (marker_x le 28)) then begin
        widget_control, State.XValLabel, SET_VALUE='Channel: ' $
                        + hk_name(hdr_hk_chan[marker_x-1])
;                       + strmid(hk_name(hdr_hk_chan[marker_x-1]),0,26)
   	widget_control, State.ZValLabel, SET_VALUE='HK Value: ' $
                        + string(image(marker_x,marker_y),f='(g12.6)') $
                        + ' ' + hk_units(hdr_hk_chan[marker_x-1])
  ...
  state.CutsMenu: $
   ...
   case Event.value of
    ...
    2: begin
       ...
       if ((marker_x ge 1) and (marker_x le 28)) then begin
           pv_title = 'HK CH' + strtrim(string(hdr_hk_chan[marker_x-1]),2) $
                + ': ' + hk_name(hdr_hk_chan[marker_x-1])
	   if (time[0] gt 1.0e8) then begin
		...
		modtime = float(time)
           	Xtit = 'S/C Time (s)'
	   endelse
           Ytit = hk_units(hdr_hk_chan[marker_x-1])
	   ...
	   if (marker_x eq 0) then begin
		...
		case marker_x of 
		    29: begin
 		          pv_title = 'Column 29: XRT State Flag'
 		          Ytit = 'XRT State'
			end
		    30: begin
 		          pv_title = 'Column 30: XRT Mode Flag'
 		          Ytit = 'XRT Mode'
			end
		    31: begin
 		          pv_title = 'Column 31: ACS Is_Settled Flag'
 		          Ytit = 'Is_Settled Flag'
			end
		    32: begin
 		          pv_title = 'Column 32: ACS Is_In_10_Arcmin Flag'
 		          Ytit = 'Is_In_10_Arcmin Flag'
			end
		    33: begin
 		          pv_title = 'Column 33: ACS Is_In_SAA Flag'
 		          Ytit = 'Is_In_SAA Flag'
			end
		    34: begin
 		          pv_title = 'Column 34: ACS Safe Mode Flag'
 		          Ytit = 'Safe Mode Flag'
			end
		endcase
	   	if (time[0] gt 1.0e8) then begin
...
;--------------------------------------------------------------------------
       ...
       print, 'unknown event in pass1_hk_viewer'
       help,/ST,Event
       end
endcase
...
;;; Widget Event Handler Procedure
;==========================================================================
PRO hkViewerEvent, Event

event = hkViewerEventFn( Event )
return
end
...
;==========================================================================
FUNCTION pass1_hk_viewer, image, ID=id, PARENT=parent, TITLE=title, $
		      UVALUE=uvalue, GROUP=group, UNITS=units, _EXTRA=extra

;===========================================================================
;===========================================================================

;; If the user supplies a widget ID, then he wants to modify an existing
;; pass1_hk_viewer widget, not create a new one.
if (0 NE n_elements(id)) then begin
  ModifyhkViewer, id, image, _EXTRA=extra
  return, id
endif
...
if NOT keyword_set(Parent) then begin
  ...
  xmanager, 'pass1_hk_viewer', top_base, GROUP_LEADER=Group, $
	    EVENT_HANDLER='hkViewerEvent', /JUST_REG
endif

;; Pass the image to ModifyhkViewer so it can be stored and drawn.
ModifyhkViewer, top_base, image, _EXTRA=extra
