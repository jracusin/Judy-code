PRO wt_bias_temp, infile,frameno,bias_med,ccd_t
...
;  if n_elements(where(round_ccd_t eq max_t)) lt 2 then max_t=max_t-1
  for temp_incr=-mt,max_t-1 do begin
...
;  device,filename=psname,xs=20,ys=15,/col
  ...
  ind=where(ccd_ave_plot ne 0.,nind)
