pro process_pu_frame_hdr, buffer_pointer, error_code
; $Id: process_pu_frame_hdr.pro 0.0 1995/12/21 17:25:30 burrows Exp $

;name	: process_pu_frame_hdr
;author	: Dave Burrows
;date	: 02/10/02
;lang	: IDL

;purpose: This routine processes the XRT CCD Frame Header for 
;	Piled-Up Photodiode mode.  It extracts
...
;	error_code	
;	    0	no errors
;	   91	PROCESS_PU_FRAME_HDR: EOF encountered before end of CCD header
;
;   Rev:
...
;	06/27/03 by DNB: fixed error in print statement for N>LLD
;		(was printing N_events instead of N_pixels_over_LLD)
;	05/21/03 by DNB: fixed error in frame start times
;	04/08/03 by DNB: resynchronized frame counters
;	03/27/03 by DNB: dropped synchronization between on-board and internal CCD frame number
;	09/04/02 by DNB: created from process_lr_frame_hdr.pro
...
;	data block
if (buffer_pointer ge (data_buffer_size-158+20)) then begin
	print,''
...
if (n_frames lt 20) then begin
	print_debug,12,'*****************************************'
	print_debug,12,'PU_PU_PU_PU_PU_PU_PU_PU_PU_PU_PU_PU_PU_PU'
	print_debug,12,'PROCESS_PU_FRAME_HDR: Processing Piled-Up Photodiode Mode Frame Hdr ' $
...
if (frame_num ne ccd_frame) then begin
	...
	print_error,0,1,'PROCESS_PU_FRAME_HDR: ERROR in CCD Frame #: found '+ $
...
if (xrt_mode ne 4) then print_error,0,1,'PROCESS_PU_FRAME_HDR: wrong xrt_mode found for Piled-Up Photodiode mode: mode flag is ' + strtrim(string(xrt_mode),2)
...
if ((waveform lt 0) or (waveform gt 255)) then begin
	if (n_elements(oldwaveform) eq 0) then begin
		print,''
		print,'PROCESS_PU_FRAME_HDR: *** INVALID WAVEFORM NUMBER FOUND: ', waveform, ' ***'
...
if (readout_stop_time lt readout_start_time) then print_error,0,1, $
    'PROCESS_PU_FRAME_HDR: WARNING: Readout START time > Readout STOP time!'
...
row_time = delta/601		; nrow = 602, times are in row 1 and 602
...
printf,lustats, format='(i10,'' PU '', 5f20.6,5f10.6)', $
...
print_debug,0,'  Frame #' + strtrim(string(frame_num),2) $
	+ ':  PUPD mode @ Satellite_time = ' + strtrim(string(satellite_time),2) $
	+ ',  N > LLD = ' + strtrim(string(N_pixels_over_LLD),2) $
...
if (amp ne amp_num) then print_error,0,1,$
	'PROCESS_PU_FRAME_HDR: Wrong Amp for this waveform number - waveform = ' $
...
if (n_frames lt 20) then begin
	...
	if (idebug ge 25) then begin
		print,'PROCESS_PU_FRAME_HDR: HK = ', hk_array
		printf,lulog,'PROCESS_PU_FRAME_HDR: HK = ', hk_array
	endif
endif
...
sxaddhist, ' ', ccd_hdr
sxaddhist, 'XRT Piled-up Photodiode Mode CCD Frame', ccd_hdr
...
sxaddpar, ccd_hdr, 'T0', first_pixel, 'Satellite time of first pixel'
