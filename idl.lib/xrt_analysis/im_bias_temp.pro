PRO im_bias_temp, infile,frameno,bias_med,ccd_t
...
;ave_evthresh=fltarr(13)
  ...
  for temp_incr=-mt,max_t do begin
     index=where(ccd_t gt temp_incr-2 and ccd_t lt temp_incr+2)
...
;  !p.multi=0
  ...
  openw,nomelogico,infile+'_ccdtemp_bias.dat',/get_lun
  tabprint=[transpose(ccd_ave_plot),transpose(ave),transpose(avesigma),transpose(ave_evthresh)]
  printf,nomelogico,'CCD temperature and average bias and sigma (from 20 header pixels) for IM frames'
