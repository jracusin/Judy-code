pro process_pc_frame_hdr, buffer_pointer, error_code
; $Id: process_pc_frame_hdr.pro 0.0 1995/12/21 17:25:30 burrows Exp $

;name	: process_pc_frame_hdr
;author	: Dave Burrows
;date	: 02/10/02
;lang	: IDL

;purpose: This routine processes the XRT CCD Frame Header for 
;	photon-counting mode.  It extracts
...
;	error_code	
;	    0	no errors
;	   91	PROCESS_PC_FRAME_HDR: EOF encountered before end of CCD header
;
;   Rev:
;   06/30/05 by JLR: changed ra,dec, & roll to doubles rather than floats
;	04/20/05 by DNB: updated for Bld 8.9: added more info to hdr version 3
;	04/10/05 by DNB: updated for Bld 8.8 Patch B: pixels added to header
;	06/01/04 by DNB: fixed bug with labels on IMAGE mode frames
...
;	09/02/02 by DNB: corrected error in frame time calculation.
;	08/12/02 by DNB: changed some error labels to indicate correct routine,
;		added error message for incorrect xrt mode.
...
;	data block
if (buffer_pointer ge (data_buffer_size-228+20)) then begin
	print,''
...
if (n_frames lt 20) then begin
	...
	print_debug,12,'PROCESS_PC_FRAME_HDR: Processing Photon-Counting Frame Hdr ' $
...
if (frame_num ne ccd_frame) then begin
	...
	print_error,0,1,'PROCESS_PC_FRAME_HDR: ERROR in CCD Frame #: found '+ $
...
if (xrt_mode ne 7) then print_error,0,1,'PROCESS_PC_FRAME_HDR: wrong xrt_mode found for Photon Counting mode: mode flag is ' + strtrim(string(xrt_mode),2)
...
if ((waveform lt 0) or (waveform gt 255)) then begin
	if (n_elements(oldwaveform) eq 0) then begin
		print,''
		print,'PROCESS_PC_FRAME_HDR: *** INVALID WAVEFORM NUMBER FOUND: ', waveform, ' ***'
...
if (readout_stop_time lt readout_start_time) then print_error,0,1, $
    'PROCESS_PC_FRAME_HDR: WARNING: Readout START time > Readout STOP time!'
...
row_time = delta/(nrow-1)
if (last_mode eq xrt_mode) then begin
	xfer_time = readout_start_time - last_stop_time - row_time 
	printf,lulog,'PROCESS_PC_HEADER: calculated XFER time is ', xfer_time
endif else xfer_time = 10.04D-3

if (n_frames lt 20) then print_debug,15,'     Delta, Row_time, xfer_time:   ' $
	+ string(delta) + string(row_time) + string(xfer_time)
...
frame_start_time = readout_start_time - row_time - xfer_time - exposure_time
frame_stop_time = readout_start_time - row_time - xfer_time
satellite_time = (frame_start_time + frame_stop_time)/2.0 ; use mid-point of CCD exposure

printf,lustats, format='(i10,'' PC '', 5f20.6,5f10.6)', $
...
N_events = extract_int(buffer_pointer)

print_debug,0,'  Frame #' + strtrim(string(frame_num),2) $
	+ ':    PC mode @ Satellite_time = ' + strtrim(string(satellite_time),2) $
	+ ',  N_events = ' + strtrim(string(N_events),2) $
...
split_threshold = extract_int(buffer_pointer)
ring_threshold = extract_int(buffer_pointer)

N_single = extract_int(buffer_pointer)
N_split = extract_int(buffer_pointer)
N_triple = extract_int(buffer_pointer)
N_quad = extract_int(buffer_pointer)

window_halfwidth = extract_int(buffer_pointer)
window_halfheight = extract_int(buffer_pointer)

amp = extract_byte(buffer_pointer)
if (amp ne amp_num) then print_error,0,1,$
	'PROCESS_PC_FRAME_HDR: Wrong Amp for this waveform number - waveform = ' $
	+ strtrim(string(waveform),2) + ', amp = ' + strtrim(string(fix(amp)),2)

baseline_offset = extract_int(buffer_pointer)

pixel_overflow = extract_int(buffer_pointer)
if (pixel_overflow ne 0) then print_error,0,1,'PROCESS_PC_FRAME_HDR: WARNING - pixel value overflow in CCD frame ' $
	+ strtrim(string(frame_num),2) + ' = ' $
	+ strtrim(string(pixel_overflow),2)

pixel_underflow = extract_int(buffer_pointer)
if (pixel_underflow ne 0) then print_error,0,1,'PROCESS_PC_FRAME_HDR: WARNING - pixel value underflow in CCD frame ' $
	+ strtrim(string(frame_num),2) + ' = ' $
	+ strtrim(string(pixel_underflow),2)


hdr_version = extract_byte(buffer_pointer)
if (hdr_version gt 0) then begin
	buffer_pointer = buffer_pointer + 5	; skip spare bytes
	N_Hdr_Pix = extract_byte(buffer_pointer)
	Bias_Pixels = intarr(N_Hdr_Pix)
	for j=0,19 do begin
		Bias_Pixels[j] = extract_int(buffer_pointer)
	endfor	
	if (hdr_version ge 3) then begin
		cps_win_start_col = extract_int(buffer_pointer)
		cps_win_start_row = extract_int(buffer_pointer)
		cps_win_ncol = extract_int(buffer_pointer)
		cps_win_nrow = extract_int(buffer_pointer)
		cps_win_threshold = extract_int(buffer_pointer)
	endif
endif else begin
	; Skip spare bytes
...
if (n_frames lt 20) then begin
	print_debug,12,'     Event thresholds: Event = ' $
		...
		+ ', Split = ' + strtrim(string(split_threshold),2) $
		+ ', Ring = ' + strtrim(string(ring_threshold),2) $
		+ ', ULD = ' + strtrim(string(ULD),2)

	print_debug,12,'     Baseline_offset = ' $
		+ strtrim(string(baseline_offset),2) $
		+ ',     Pixel overflow = ' $
		+ strtrim(string(pixel_overflow),2) $
		+ ',     Pixel Underflow = ' $
		+ strtrim(string(pixel_underflow),2)
	...
	print_debug,12,'          N_events = ' + strtrim(string(n_events),2)
	...
	print_debug,12,'          # single events = ' + strtrim(string(n_single),2)
	print_debug,12,'          # double events = ' + strtrim(string(n_split),2)
	print_debug,12,'          # triple events = ' + strtrim(string(n_triple),2)
	print_debug,12,'          # quad events = ' + strtrim(string(n_quad),2)
endif

if ((N_split + N_single + N_triple + N_quad) gt N_events) then begin
	print_error,0,1,'PROCESS_PC_FRAME_HDR: *** ERROR IN CCD FRAME HEADER ***'
	print_error,0,1,'                     : N_SINGLE + N_SPLIT <> N_EVENTS'
	print_error,0,1,'                     : N_SINGLE = '+strtrim(string(n_single),2)
	print_error,0,1,'                     : N_SPLIT = '+strtrim(string(n_split+n_triple+n_quad),2)
	print_error,0,1,'                     : N_EVENTS = '+strtrim(string(N_events),2)
endif
...
if (n_frames lt 20) then begin
	...
	if (idebug ge 25) then begin
		print,'PROCESS_PC_FRAME_HDR: HK = ', hk_array
		printf,lulog,'PROCESS_PC_FRAME_HDR: HK = ', hk_array
	endif
endif
...
sxaddpar, ccd_hdr, 'DATAMODE', readout_mode, 'XRT CCD in Photon-Counting Mode'
sxaddhist, ' ', ccd_hdr
sxaddpar, ccd_hdr, 'CREATOR',  'PASS1 (photon_counting), '+version

sxaddhist, ' ', ccd_hdr
sxaddhist, 'XRT Photon-Counting Mode CCD Frame', ccd_hdr
sxaddpar, ccd_hdr, 'HDR_VER', fix(hdr_version), 'Header version number'
...
if (hdr_version ge 3) then begin
	sxaddpar,ccd_hdr,'CPS_WCOL',cps_win_start_col,'Window Start Column for Count Rate Calculation'
	sxaddpar,ccd_hdr,'CPS_WROW',cps_win_start_row,'Window Start Row for Count Rate Calculation'
	sxaddpar,ccd_hdr,'CPS_NCOL',cps_win_ncol,'Number Columns in Window for Count Rate Calculation'
	sxaddpar,ccd_hdr,'CPS_NROW',cps_win_nrow,'Number Rows in Window for Count Rate Calculation'
	sxaddpar,ccd_hdr,'CPS_LLD',cps_win_threshold,'Threshold (DN) for Count Rate calculation'
endif
...
EVENT_HEADER.ISPLIT_THRESHOLD = split_threshold 
EVENT_HEADER.EVENT_THRESHOLD = event_threshold
EVENT_HEADER.SPLIT_THRESHOLD = split_threshold 
EVENT_HEADER.XMIN = 300 - window_halfwidth + 2	; counting from 0
EVENT_HEADER.YMIN = 300 - window_halfheight + 2   ; counting from 0
EVENT_HEADER.XMAX = 299 + window_halfwidth - 2
EVENT_HEADER.YMAX = 299 + window_halfheight - 2
EVENT_HEADER.CCD = 1
...
sxaddpar,ccd_hdr,'SPTHRESH',split_threshold,'Split Event threshold (DN)'
sxaddpar,ccd_hdr,'RING',ring_threshold,'Outer Ring threshold (DN)'


sxaddpar,ccd_hdr,'BSLN_OFF',baseline_offset,'Baseline_offset (DN)'
case baseline_correction_mode of
  0: sxaddpar, ccd_hdr, 'BASELINE', 'NONE', 'No baseline correction performed'
  1: sxaddpar, ccd_hdr, 'BASELINE', 'CORNER', 'Subtracted baseline determined from corner pixels'
endcase

sxaddpar,ccd_hdr,'OVERFLOW',pixel_overflow,'Pixel Value Overflow (DN)'
sxaddpar,ccd_hdr,'UNDRFLOW',pixel_underflow,'Pixel Value Underflow (DN)'
...
sxaddpar,ccd_hdr,'N_TRIPLE', N_triple,'Number of three-pixel events'
sxaddpar,ccd_hdr,'N_QUAD', N_quad,'Number of four-pixel events'
