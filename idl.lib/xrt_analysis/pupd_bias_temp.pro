PRO pupd_bias_temp, infile,frameno,bias_med,ccd_t
...
;forprint, frame
  ...
  for i=0,nf-1 do begin
     ...
     bias_thresh[i]=sxpar(hd0,'BIAS_TH')
;     for ii=0,19 do begin
;        pix_num=string(ii)
;        pixel_name='PIXEL'+strtrim(pix_num,2)
;     bias_pixel[ii]=sxpar(hd0,pixel_name)
;     endfor
;     index_median=where(bias_pixel lt 1800,n_pix_nosaa)
     
;     if (n_pix_nosaa ne 0) then begin
;        bias_median_values=bias_pixel(index_median)
     bias_med[i]=sxpar(hd0,'MEDIAN') ;median(bias_median_values)
     sigma[i]=sxpar(hd0,'STDEV') ;stdev(bias_median_values)
...
;  device,filename=psname,xs=20,ys=15,/col
  tito='Bias level subtracted on board vs CCD temp - '+infile
...
;  device,filename=psname,xs=20,ys=15,/col
  ...
  plot,ccd_t,bias_med,psym=5,ytit='Median Value (DN)',tit=tito,xtick_get=xtick,xminor=4,xticklen=0.05,xrange=xrange
  multiplot
  ...
  multiplot
  plot,ccd_ave_plot(ind),ave(ind),ytit='Ave Median Value (DN)',psym=6,xrange=xrange,xminor=4,xticklen=0.05
  multiplot
...
;plot,ccd_ave_plot(ind),evthresh(ind),xtit='CCD temp',ytit='LLD',psym=1,yrange=[min(evthresh(ind))-1.,max(evthresh(ind))+1.]
  multiplot
  multiplot
  plot,frameno,bias_med,psym=4,xtit='Frame Number',ytitle='Median Value (DN)',xrange=[min(frameno),max(frameno)],xtickformat='(i7)',xminor=5,xticklen=0.05
  multiplot,/reset
...
;  !p.multi=0
  ...
  openw,nomelogico,'pupd_ccdtemp_bias.dat',/get_lun
  tabprint=[transpose(ccd_ave_plot),transpose(ave),transpose(avesigma),transpose(ave_evthresh)]
