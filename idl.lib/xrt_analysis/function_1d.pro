...
;========================================================================
;;;
;;; Function_1D Widget: @(#)function_1d.pro	1.41
;;;
...
dataset = { index:0B, name:'', description:'', hidden_flag:0, disable_delete:0,$
	    ...
	    plot: {color:'white', psym:3, line:0}, $
...
state = { parent:parent, $ 
	...
	;Dataset structures
	selected_name:'', datasets: replicate( dataset, 40 ), $
	
	;ROI parameters
	roi_params: {xl:1., xh:0.}, $

	;Other state information.
	legend_style:0, show_errors:0, share_params:1, fitnum:1, $
...
color_manager, COLOR_NAMES=color_names  & nc = n_elements(color_names)
...
if (dataset.filter_stale) then begin
  ...
  if (dataset.num_data_points EQ 0) then begin
    ...
    y_max = max(*dataset.y_filtered_data, MIN=y_min)
  endelse  
...
if (yaxis.default_flag) then begin
 case mode of
  0: set_yrange = [min( plotsets.y_min ), max( plotsets.y_max )]

  1: begin
     min_y =  1.0E10
     max_y = -1.0E10
     for ii = 0, n_elements(plotsets)-1 do begin
       int_max = max( *plotsets[ii].integral.samples, MIN=int_min )
       min_y = min( [min_y, int_min] )
       max_y = max( [max_y, int_max] )
     endfor
 ...
 ;-------------------------------------------------------------------------
  ...
  for ii = 0, n_elements(plotsets)-1 do begin
    ...
    if (dataset.num_data_points GT 0) then begin
      ...
      if ((*st).show_errors AND dataset.error_available) then begin
        errplot, *dataset.x_filtered_data, $
        	 *dataset.y_filtered_data - *dataset.e_filtered_data, $
        	 *dataset.y_filtered_data + *dataset.e_filtered_data
      endif
    endif
  endfor
     ...
     case (mode) of
      0: $
       ...
       msg = string( x_pt, y_pt, $
                     f='("Function sample at ",G11.5," = ",G11.5)')
       end
      1: $
       begin
       msg = string( x_pt, y_pt, $
                     f='("Integral sample at ",G11.5," = ",G11.5)')
       end
     endcase 
    ...
    2: $ ; SAVE FUNCTION as FITS
     ...
     if (pathname NE '') then begin
       ...
       if (Error NE 0) then begin
         ...
         sxaddpar, pheader, "CREATOR", "function_1d.pro, Version 1.41"
	 ...
	 writefits, pathname, no_data, pheader
         ...
         sxaddpar, fits_header, "CREATOR", "function_1d.pro, Version 1.41"
...
; Edit button: $
  (*st).edit_button: $
   ...
   ;  PLOT PROPERTIES
    ...
    f3 = '0, DROPLIST, Omit|Left|Center|Right, SET_VALUE=' + $
   	string((*st).legend_style) + ', LABEL_LEFT=Legend, TAG=legend_style'
   	
    f5 = '2, DROPLIST, Omit Errors|Show Errors, SET_VALUE=' + $
   	string((*st).show_errors) + ', TAG=show_errors'
   	
     form = [form,b0,l0,f1,f4,f2,f5,f3]
...
;; Find the specified dataset or a spot to put a new one.
  if (NOT keyword_set(dataset_name)) then dataset_name = 'Y vs X'
