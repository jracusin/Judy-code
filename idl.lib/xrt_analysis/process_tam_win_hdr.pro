pro process_tam_win_hdr, buffer_pointer, error_code
; $Id: process_tam_image_hdr.pro 0.0 1995/12/21 17:25:30 burrows Exp $

;name	: process_tam_win_hdr
;author	: Dave Burrows
;date	: 02/10/02
;lang	: IDL

;purpose: This routine processes the XRT TAM Window Frame Header.  It extracts
;	data from this header for the output FITS header.
;	It returns an error code which is 0 if no errors are found:
;	error_code	
;	    0	no errors
;	   76	Packet is not LDP page 1
;	   77	Found redundant LDP page 1
;	   78	Packet is not a TAM Image Header
;	   91	PROCESS_TAM_WIN_HDR: EOF encountered before end of CCD header
;
;   Rev:
;	08/09/04 by DNB: fixed error in unpacking UTC factors (was treated as 
;			unsigned, should have been signed)
;	05/31/04 by DNB: added LED_SELECT and TAM_LLD to DTAS file
;	05/21/04 by DNB: added support for DTAS trending
...
print_debug,0,'PROCESS_TAM_WIN_HDR: Processing TAM Windows Hdr ' $
...
if (LDP_num ne tam_frame) then begin
	...
	print_error,0,1,'PROCESS_TAM_WIN_HDR: ERROR in TAM Frame #: found '+ $
...
print, 'PROCESS_TAM_WIN_HDR: Product # = ', LDP_num,$
		'   Page # = ', LDP_page, '   Pages in file = ', $
				pages_in_file
printf,lulog, 'PROCESS_TAM_WIN_HDR: Product # = ', LDP_num,$
		'   Page # = ', LDP_page, '   Pages in file = ', $
				pages_in_file

if (LDP_page ne 1) then begin
	print_error,0,1,'PROCESS_TAM_WIN_HDR: LDP page number is not 1...'
...
if (LDP_num eq LDP_num_old) then begin	; redundant LDP header
	print_error,0,1,'PROCESS_TAM_WIN_HDR: found redundant LDP header -- skipping this packet'
	LDP_page_old = LDP_page
	LDP_num_old = LDP_num
	error_code = 77
	tam_frame = 0
...
data_collection_sec = extract_long(buffer_pointer)
data_collection_subsec = extract_int(buffer_pointer)
data_collection_time = data_collection_sec + data_collection_subsec*20.0D-6
...
tam_image_time = data_collection_time + readout_utc_time

print,format='(a, f20.6, f10.6)', 'TAM image time and UTCF: ', tam_image_time, $
		readout_utc_time
printf,lulog,format='(a,f20.6,f10.6)','TAM image time and UTCF: ', tam_image_time, $
		readout_utc_time

delta = tam_image_time - last_time

if (tam_frame eq first_frame) then first_time = tam_image_time $
else last_time = tam_image_time

	if (idebug ge 35) then begin
		print,'PROCESS_TAM_WIN_HDR: Observation segment, Target ID = ', $
				obs_segment, target_id
		print,format='(a,''0x'',z8.8)', $
			'PROCESS_TAM_WIN_HDR: Data collection time seconds = ', $
				data_collection_sec
		print,format='(a,''0x'',z8.8)',$
			'PROCESS_TAM_WIN_HDR: Data collection time subsec  = ', $
				data_collection_subsec
		print,'PROCESS_TAM_WIN_HDR: Data collection time = ', data_collection_time, ' s'
		print,format='(a,''0x'',z8.8)', $
			'PROCESS_TAM_WIN_HDR: Data collection UTC seconds = ', readout_utc_sec
		print,format='(a,''0x'',z8.8)',$
			'PROCESS_TAM_WIN_HDR: Data collection UTC subsec  = ', readout_utc_subsec

		printf,lulog,'PROCESS_TAM_WIN_HDR: Observation segment, Target ID = ', $
				obs_segment, target_id
		...
		printf,lulog,format='(a,''0x'',z8.8)',$
			'PROCESS_TAM_WIN_HDR: Data collection time subsec  = ', $
				data_collection_subsec
		printf,lulog,'PROCESS_TAM_WIN_HDR: Data collection time = ', data_collection_time, ' s'
		printf,lulog,format='(a,''0x'',z8.8)', $
			'PROCESS_TAM_WIN_HDR: Data collection UTC seconds = ', readout_utc_sec
		printf,lulog,format='(a,''0x'',z8.8)',$
			'PROCESS_TAM_WIN_HDR: Data collection UTC subsec  = ', readout_utc_subsec
...
if (tam_frame_header_id ne '9A051111'x) then begin
	print_error,0,1,'PROCESS_TAM_WIN_HDR: ERROR - found incorrect TAM header ID = 0x' + strtrim(string(tam_frame_header_id,format='(z8.8)'),2) + ' at buffer position ' + strtrim(string(buffer_pointer),2)
	print_error,0,1, 'PROCESS_TAM_WIN_HDR: packet is not a TAM Image Header - returning to calling program'
	error_code = 78
	return
endif
...
case tam_report_mode of
	...
	else: stop, 'PROCESS_TAM_WIN_HDR: Unrecoverable error - invalid TAM report mode'
endcase
...
printf,lutam,format='(3i10,i5,2f20.6,6f10.3)',$
	tam_sequence_counter,tam_frame,target_id,obs_segment,$
		tam_image_time, delta,$
		tam_x1,tam_y1,tam_sig1,$
		tam_x2,tam_y2,tam_sig2

; write data to DTAS trending file
printf,ludtas,format='(a,8('','',f10.3))', $
	itos_time(tam_image_time),$
		tam_x1,tam_y1,tam_sig1,$
		tam_x2,tam_y2,tam_sig2,$
		led, tam_lld
...
sxaddhist, ' ', tam_hdr
sxaddhist,'*** SWIFT XRT *** TAM Windows Frame processed by PASS1 '+version, $
			tam_hdr
...
sxaddpar, tam_hdr, 'TAM_TIME', data_collection_time,'Satellite time of readout start'
