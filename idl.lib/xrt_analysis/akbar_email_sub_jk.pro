...
;  if keyword_set(noprint) then prt='N' else prt='Y'
  ...
  scvc0tab = mrdfits(filebegin+'_ahkVC0.0.stripchart_table.fits',1,h)
  s=size(scvc0tab)
  ...
  strtsampstop=''
  strtdata=''
  strtstarttime=scvc0tab[0].sctime
  strtstoptime=scvc0tab[s[1]-1].sctime
  note=''
  f165='(a,a)'
  ...
  printf,aklun,'Stripchart APID 0X484 RT Start Time(1) [format=F16.5]: ',strtstarttime,format='(a,f16.5)'
  printf,aklun,'Stripchart APID 0X484 RT Stop Time(1) [format=F16.5]: ',strtstoptime,format='(a,f16.5)'
...
;  fastmail -r kennea@astro.psu.edu -f "XRT XDS" -F kennea@astro.psu.edu akbar_email_signed.txt akbar@dbms1.gsfc.nasa.gov
